...
### Production-grade lab gate

`frontend-lab` means an independently runnable **production-grade product build**, not an automatic sandbox, mood-board, or excuse for placeholder content. Keep experimental provenance in repository docs, manifests, and internal notes—not as visitor-facing apology copy. Do not add `fictional`, `concept`, `demo`, `lab`, `sandbox`, `not for sale`, or equivalent disclaimers to the public page merely because the product was invented for the exercise. Use those labels only when the owner explicitly asks for a non-live/sandboxed surface or when a real safety, transaction, legal, consent, privacy, or authentication boundary must be disclosed.

When the owner authorizes inventing the product, establish a coherent scoped product truth before composing: audience, positioning, primary action, named capabilities, evidence/proof carrier, and conversion/availability boundary. Treat those approved facts as the lab's product model; do not undermine them with self-referential placeholder copy. If a real conversion integration is unavailable, provide a truthful content/navigation endpoint or mark the integration as an implementation blocker—never a decorative fake waitlist or checkout.

Before calling a production lab ready, run an anti-slop composition check: reject a transferable stack of oversized display headline, floating product cutout, atmospheric gradient, three interchangeable feature cards, and generic CTA when its geometry is not derived from the product object, transformation, environment, or proof. Replace the weakest generic block with an inspectable product-derived relationship. Typography is part of this gate: define roles before styling, use a display face only for its explicit semantic tier, and solve weak hierarchy or a bad wrap through content, measure, size, line-height, and structure before tightening tracking or adding atmosphere. Treat a rendered type orphan, display-face leakage, or aggressive negative tracking as a production blocker, not finishing polish.
...
For a full-page website, include the **website frame** in that anatomy map—not only the sections between Hero and CTA. Record whether the header/navigation is static, sticky, or fixed and why; prefer persistent sticky/fixed orientation when the page is long and the navigation remains useful. A desktop primary navigation must not simply disappear on mobile without an equivalent compact navigation, menu, or modal drawer. Treat the footer as a real closing information architecture surface when the product job calls for one: site directory, truthful utility controls, ownership/provenance, contact, trust, or legal links as supported by product evidence. A logo row or metadata strip alone is not a complete website footer merely because it visually ends the page. Use the Website Frame Gate in [references/brief-workflow.md](references/brief-workflow.md) for responsive and interaction proof.

For whole-page reference fidelity, keep the anatomy map as a **closure ledger** through implementation and review. Every mapped frame/section must retain its target anatomy, material, state, responsive contract, current DOM/CSS owner, implementation status, and paired rendered evidence at matched viewports/states. If any mapped legacy section or unreviewed row remains, report partial fidelity rather than a whole-page pass. A clean build, behavior tests, coherent downscaled full-page screenshot, or reviewer summary cannot close a row by itself; use section-level reference/current captures plus relevant DOM geometry, and remove or clearly mark stale QA artifacts before the final handoff. Load the whole-page closure contract in [references/reference-contracts.md](references/reference-contracts.md) when this scope applies.
...
Treat tracking as role-, typeface-, weight-, size-, and viewport-dependent. Start from normal font spacing; do not use aggressive negative letter-spacing as a portable shorthand for premium, modern, or polished typography. After rendering, inspect computed font family, weight, size, line height, letter spacing, measure, wrapping, and optical glyph separation at actual scale, then recalibrate whenever the typeface, weight, role, or breakpoint changes.
...
- Do not invent brand assets, product data, metrics, legal copy, routes, or user-owned content. The only exception is the clearly fictional placeholder access/pricing values permitted for explicitly sandboxed learning labs. Separately, when the owner authorizes an invented product, establish and preserve that scoped product model as above, without presenting invented regulated, transactional, legal, privacy, or authentication behavior as live.
...
- Rendered typography QA checks computed font family, weight, size, line height, letter spacing, measure, wrapping, and optical glyph separation for every material role at primary and narrow viewports, including mid-page section anchors rather than only the first viewport.
...
- Production-grade labs keep internal experiment/provenance labels out of visitor-facing copy unless disclosure is explicitly required, establish an owner-authorized product model before composition, and never substitute a decorative fake conversion for a real endpoint or an explicit implementation blocker.
- Production-grade labs pass an anti-slop composition and typography gate: product-derived proof outranks a transferable hero/card/gradient stack, and type roles, wraps, measure, line-height, and optical spacing are rendered and corrected before visual finishing.
