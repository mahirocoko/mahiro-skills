...
2. Explicit `callback` fails before dispatch when an exact parent or target receipt is unavailable. It launches a lightweight detached lifecycle guard, not the full result-capture watcher or a controller Monitor. The guard requires an observed `working` state rather than treating a sequence change as execution, then polls the receipt-bound target until the same `idle`/`done` state and sequence remain stable through the terminal-confirmation window (10 seconds by default). Any return to `working` resets confirmation. Accepted final-callback transport makes the guard exit silently; a missing, pending, failed, invalid, or receipt-mismatched result after stable termination records per-target attention and emits at most one metadata-only parent wake per job with the exact `recover` command. Transport acceptance never finalizes the job, so the default 30-minute one-shot silence process remains armed until parent acknowledgement and separately covers hung or unreceived work. Use `--callback-timeout 0` only for a deliberately unmanaged experiment.
...
Treat a lifecycle-guard or silence wake as attention, not completion proof. Inspect `show "$JOB_ID"` and the exact pane first, then run `recover "$JOB_ID"` from the exact parent only while callback evidence is still missing. Callback recovery does not start the watcher: it first requires a fully dispatched `running` job plus persisted guard evidence that every target was observed `working`, then requires all receipt-bound targets to remain `idle`, `done`, or `blocked` with unchanged state through a bounded confirmation window, captures bounded pane output synchronously, and rechecks receipt, status, sequence, and callback records before one locked terminal transition. It exits `3` with `recovery=deferred` and leaves callback ownership intact when dispatch is incomplete, working was never observed, a target is still `working`, has unknown state, changes state, or already has a final callback record. Use `receive`, `retry`, or `audit` for an existing callback record. A callback job with no full watcher is not reconciled as failed merely for that absence; the watcher remains the owner only for jobs routed to watcher mode. Notifications remain generic metadata only.
...
Keep the current proven source-art route until the selected replacement model is observed receiving Codex's hosted `image_generation` tool. Image input support or a higher coding-model priority alone does not establish image-generation availability.
...
tmux send-keys -t codex-task 'codex --model "gpt-6-astra" -c model_reasoning_effort=high --dangerously-bypass-approvals-and-sandbox' Enter
...
```bash
tmux send-keys -t codex-task 'codex --model "gpt-6-astra" -c model_reasoning_effort=ultra --dangerously-bypass-approvals-and-sandbox' Enter
tmux capture-pane -p -t codex-task -S -120
