...
- Antigravity (`agy`) curated choices are `claude-opus-4-6-thinking` for heavy review/reasoning, `claude-sonnet-4-6` for a balanced lane, and `gemini-3.5-flash-medium` for a faster lane.
...
- For Antigravity, ask among `claude-opus-4-6-thinking`, `claude-sonnet-4-6`, and `gemini-3.5-flash-medium`. Launch with the exact stable slug from `agy models` and verify the visible model in the pane. Agy exposes native `--effort`, but pass it only when the selected model supports that effort; `claude-opus-4-6-thinking --effort high` is invalid and silently falls back to the default model. Treat any fallback warning or visible model mismatch as launch failure, not success.
...
- Verified 2026-07-21: Cursor `agent` and the official installer are both `2026.07.17-3e2a980`. `agent models` currently exposes Composer 2.5, Fable 5, Sonnet 5, Opus 4.8, GPT-5.6 Sol/Terra/Luna, and other families. Catalog labels advertise 1M for Fable/Sonnet/Opus/GPT-5.6, while `agent about` for the active Fable session still reported `Fable 5 300K High`; treat effective context as session-dependent until a fresh launch proves otherwise. Use exact model IDs, not display shorthand.
- Verified 2026-07-21: local Antigravity `agy` is `1.1.5`. `agy models` exposes `claude-opus-4-6-thinking`, `gemini-3.1-pro-low/high`, `gemini-3.5-flash-low/medium/high`, `claude-sonnet-4-6`, and `gpt-oss-120b-medium`. Foreground tmux launch proved Opus and Sonnet stable slugs plus Flash medium; `claude-opus-4-6-thinking --effort high` warned that effort is unsupported and fell back to Gemini Flash, while the catalog-listed `gemini-3.1-pro-high` was rejected as no longer available. Therefore the curated picker excludes Gemini Pro and Opus launches omit `--effort`. The old multiline-paste behavior has not been re-tested on 1.1.5, so preserve the safe one-line/`--prompt-interactive` delivery path.
- Verified 2026-07-21: Codex CLI local and npm stable are `0.144.6`; alpha `0.145.0-alpha.28` exists but is not the default lane. `codex debug models` lists Sol/Terra/Luna/GPT-5.5/Spark as user-visible. Sol and Terra expose low through ultra, Luna low through max, and Spark low through xhigh. The current model metadata reports 272,000-token context for Sol/Terra/Luna/GPT-5.5 and 128,000 for Spark. `image_generation`, `multi_agent`, and `fast_mode` remain stable/enabled. Codex image generation still requires ChatGPT/Codex backend plus provider/model support; generated PNGs are saved under `$CODEX_HOME/generated-images/<session>/<call_id>.png`.
