...
- Do not infer direct CLI model availability, effort semantics, or defaults from Letta, OpenAI API, Cursor, or another executor's catalog. The target CLI's live catalog, launch argv, and visible pane banner are the authority for that lane.
...
`auto` selects callback only after it captures the exact parent Letta pane from `HERDR_PANE_ID` through `herdr pane get` and captures exact target receipts with authoritative non-empty agent sessions. If that proof is unavailable, it uses the existing durable watcher. Explicit `callback` fails closed before prompt dispatch or job creation when the parent or target receipt cannot be captured or when a target agent session is uninitialized/missing (instructing retry after session initialization); explicit `watcher` preserves the v1 lifecycle. Callback mode launches a lightweight detached lifecycle guard, not the full result-capture watcher or a controller Monitor. The guard must observe real `working` activity, continuously confirm a stable `idle`/`done` state, revalidate the target receipt before warning, and stay quiet when an accepted `report_ready`/`report_failed` exists; otherwise it emits at most one metadata-only parent wake per job with the exact `recover` command. Accepted transport suppresses an immediate duplicate guard wake but never finalizes the job; the default 30-minute one-shot silence deadline remains armed until parent acknowledgement and separately covers hung or unreceived work. Every target receives the same prompt bytes plus one callback footer; the record keeps separate task and dispatch SHA-256 hashes.

Callback receipts bind pane, workspace, tab, terminal, Herdr session/socket, available non-secret Letta identifiers, and authoritative target agent session. Before every send, receive, or report finalization, receipts are revalidated—never trusting caller-supplied names or allowing same-pane session replacement to bypass revalidation; legacy receipts missing session identity fail closed. The ACL is exactly parent plus job targets: parent may message targets; targets may message parent or siblings; outsiders, replaced sessions, and stale receipts fail. Named targets wake through `agent.prompt`. The exact parent Letta pane wakes through one atomic single-line metadata-only `pane.run`, because the parent is not an active named Herdr agent. Both are best-effort transport: accepted delivery is not receipt or proof, and there is no tmux fallback.
...
Treat a callback-guard or silence wake as an attention signal, not proof that the lane ended. Inspect the durable job and exact pane first, then run `recover` from the exact parent only when callback evidence is still missing. Callback recovery does not spawn a watcher: it first requires a fully dispatched `running` job plus durable guard evidence that every target was observed `working`, then requires the receipt-bound targets to remain terminal through a stable window, captures their bounded output synchronously, and rechecks receipt, status, sequence, and callback records before one job-lock terminal transition. It exits with `recovery=deferred` and keeps callback ownership when dispatch is incomplete, working was never observed, any target is still `working`, is unknown, changes state, or already has a final callback record. Use `receive`, `retry`, or `audit` for an existing callback record rather than fallback capture. Callback jobs are not reconciled as failed merely because they have no full watcher; old watcher/v1 jobs remain compatible. `list`, `show`, `wait`, and `collect` expose durable job state, while `collect` remains the controller's result-reading step. Notifications, wakes, and completion lines contain metadata only—never prompt, body, result, or failure-summary text. The watcher fallback does not inject a new message into the current Letta conversation. Do not launch `letta -p`, type into the parent pane, or create a second turn manually.
...
- `scripts/herdr-jobs.py` - callback-primary Herdr job registry, detached lifecycle guard, durable message ledger, synchronous callback recovery, watcher-mode fallback, and collection CLI
