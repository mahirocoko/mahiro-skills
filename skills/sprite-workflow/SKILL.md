...
- Declare source authorship before dispatch: `imagegen-required`, `manual-rig-allowed`, or `diagnostic-only`. When imagegen is required, manual transforms, Pillow rigs, procedural redraws, and copied/rotated master pixels cannot satisfy the source contract; return a blocker instead of quietly changing lanes.
...
- For Move/Basic/Hurt/Active and other action families, frame uniqueness, stable anchors, and jitter PASS do not prove authored motion. Require visibly differentiated anticipation/action/contact/recovery poses at runtime size; static or near-static rows fail even when every file/hash differs.
...
- `assemble-approved-atlas.py` — deterministically assemble hashed `production-approved` frame manifests without scaling, trimming, rotation, or implicit approval; missing source-authorship provenance fails unless a standalone legacy input uses `--allow-legacy-provenance` with a hash-bound review record.
...
3. Create a job with `new-job.py`; include literal target repo, kind, frame size, explicit frame count, action/direction, content and anchor policy, source lane, source requirement, provenance usage, and stable lineage IDs where available. `imagegen-required` jobs must retain a hash-bound provider receipt and raw generated source artifacts; `diagnostic-only` jobs can never be promoted as production assets.
...
5. For hard animation/effect jobs, prefer one action at a time and consider Image Cockpit-style parallel candidate lanes (`--tournament-candidates 3`) with isolated outboxes and a winner-selection QA pass. Chain only human-selected earlier actions as family references for later actions. Dispatch subagents with literal paths, and never let a worker invent a custom compositor/manual rig when the job says `sprite-generate` or `imagegen-required`. Use `references/subagent-prompts.md` for role templates.
...
8. Treat the untouched provider raster as source material rather than the runtime sheet. Do not fail a recoverable candidate solely because the provider returned a larger canvas or slightly non-uniform chroma; preserve the raw bytes/receipt and mark normalization required. For chroma-key imagegen outputs, run `extract-chroma-sheet.py` instead of ad-hoc cleanup. Pass `--provider-receipt` with an `imagegen-required` `--source-job`; extraction fails before work if the selected input is not hash-bound by that receipt, then preserves the receipt and copied raw sources in the normalized manifest. Its `--key-tolerance` is normalized `[0,1]`. It supports horizontal and explicit row-major 2D grids; component recovery fails closed rather than silently falling back, but a human-reviewed fixed grid remains a valid explicit choice. Compare dicut modes when needed, then run manifest, contact, alpha-background, body/FX, enclosed-hole, center/bounds/bottom, and motion QA.
...
10. Review raw versus normalized target-size output, correction amounts, adjacent frames, light/dark/checker alpha, motion progression, pose differentiation, equipment continuity, and one-cycle seam. A mechanical tournament score never chooses the visual winner: reject detached fragments, identity drift, action duplication, weak silhouettes, or wrong semantics even when geometry QA ranks them first. For action families, explicitly reject a transform-only rig when the job requires newly authored poses. For separate raster FX, also review event-timed body/effect composites and effect alpha, overdraw, bounds, and geometry overlays without baking guides. Script warnings are evidence, not aesthetic approval.
