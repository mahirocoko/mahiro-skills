...
function verifyTrustedCustomGemUploadReceipt(request) {
  ...
  return new Promise((resolve, reject) => {
    ...
    chrome.runtime.sendMessage(MAHIRO_BROWSER_CONTROL_EXTENSION_ID, payload, (response) => {
      clearTimeout(timer);
      if (chrome.runtime.lastError || response?.ok !== true || response?.result?.consumed !== true || response.result.receiptId !== request.uploadReceipt.receiptId) {
...
async function navigateAndVerifyCustomGemTab(tabId, deadlineAt) {
  ...
  while (Date.now() < deadlineAt) {
    ...
    if (current?.url && current.url !== CUSTOM_GEM_URL && Date.now() - startedAt > 5e3) {
...
\`\`\``);
  ...
  const sendReadyDeadline = Math.min(deadlineAt, Date.now() + 1e4);
...
function requestTrustedCustomGemSend(request) {
  ...
  return new Promise((resolve, reject) => {
    ...
    chrome.runtime.sendMessage(MAHIRO_BROWSER_CONTROL_EXTENSION_ID, payload, (response) => {
      clearTimeout(timer);
      if (chrome.runtime.lastError || response?.ok !== true || response?.result?.clicked !== true || response.result.receiptId !== request.uploadReceipt.receiptId) {
