...
// skills/gemini/extension/custom-gem-browser-command.js
...
var CUSTOM_GEM_FOLLOWUP_ACTION = "gem_followup_v1";
var CUSTOM_GEM_FOLLOWUP_COMMAND_VERSION = "custom-gem-browser-followup-command-v1";
var CUSTOM_GEM_REVISION_RESULT_VERSION = "custom-gem-browser-revision-result-v1";
var CUSTOM_GEM_FOLLOWUP_AUTHORIZATION_RESULT_VERSION = "custom-gem-followup-authorization-result-v1";
...
var CUSTOM_GEM_ID = CUSTOM_GEM_TARGETS[0].gemId;
var CUSTOM_GEM_URL = CUSTOM_GEM_TARGETS[0].gemUrl;
var CUSTOM_GEM_TARGET_BY_URL = new Map(CUSTOM_GEM_TARGETS.map((target) => [target.gemUrl, target]));
function normalizeCustomGemTarget(gemUrl) {
...
function getCustomGemSendReadiness(sourceCount) {
  ...
  const stableMs = CUSTOM_GEM_BASE_SEND_STABLE_MS + Math.max(0, sourceCount - 2) * CUSTOM_GEM_EXTRA_SOURCE_STABLE_MS;
  return {
    stableMs,
...
function verifyImageSignature(bytes, mimeType) {
...
function summarizeGemSources(sources) {
...
async function validateGemStartRequest(input) {
  ...
  if (input.message !== input.message.trim() || input.message.includes("\0")) {
  ...
  return {
    ...
    request: {
      requestId,
      tabId: input.tabId === void 0 || input.tabId === null ? void 0 : input.tabId,
      gemId: target.gemId,
...
async function validateGemSubmitRequest(input) {
  const requestId = input?.requestId ?? null;
  const allowedKeys = [
    "action",
    "currentUrl",
...
async function validateGemFollowupRequest(input) {
  const requestId = input?.requestId ?? null;
  const allowedKeys = [
    "action",
  ...
  const receiptKeys = ["browserControlExtensionId", "commandId", "receiptId", "version"];
  if (!authorizationReceipt || typeof authorizationReceipt !== "object" || Array.isArray(authorizationReceipt) || Object.keys(authorizationReceipt).length !== receiptKeys.length || Object.keys(authorizationReceipt).some((key) => !receiptKeys.includes(key)) || authorizationReceipt.version !== CUSTOM_GEM_FOLLOWUP_AUTHORIZATION_RESULT_VERSION || typeof authorizationReceipt.browserControlExtensionId !== "string" || !CHROME_EXTENSION_ID_RE.test(authorizationReceipt.browserControlExtensionId) || !CUSTOM_GEM_BROWSER_CONTROL_EXTENSION_IDS.includes(authorizationReceipt.browserControlExtensionId) || typeof authorizationReceipt.commandId !== "string" || !SAFE_ID_RE.test(authorizationReceipt.commandId) || typeof authorizationReceipt.receiptId !== "string" || !SHA256_RE.test(authorizationReceipt.receiptId)) {
...
var isExactCustomGemConversationUrl = (gemUrl, value) => normalizeCustomGemTarget(gemUrl)?.gemUrl === gemUrl && new RegExp(`^${escapeRegExp(gemUrl)}/[a-f0-9]{8,64}$`, "i").test(String(value || ""));
...
var TOPIC_RSP = "claude/browser/response";
var TOPIC_STATUS = "claude/browser/status";
var client = null;
var connected = false;
...
var customGemReplayCache = /* @__PURE__ */ new Map();
var customGemInFlight = /* @__PURE__ */ new Map();
var customGemSubmitInFlight = /* @__PURE__ */ new Map();
var customGemFollowupInFlight = /* @__PURE__ */ new Map();
var customGemTabLocks = /* @__PURE__ */ new Map();
var customGemQuarantinedTabs = /* @__PURE__ */ new Map();
var customGemDurableQueue = Promise.resolve();
...
function sendResponse(id, data) {
  ...
  const isExactRevisionSuccess = data?.version === CUSTOM_GEM_REVISION_RESULT_VERSION && data?.state === "succeeded";
  const payload = JSON.stringify(isExactRecoverySuccess || isExactRevisionSuccess ? data : {
    id,
    ...data,
    ts: Date.now()
  });
...
async function handleCommand(cmd) {
  ...
  if (cmd.action === "reload_extension_v1") {
    ...
    sendResponse(cmd.id, { success: true, reloading: true });
  ...
  try {
    switch (cmd.action) {
      case "ping":
        ...
        break;
      case "gem_start_v1":
        result = await handleCustomGemStart(cmd);
        break;
      case "gem_submit_v1":
      ...
      case "gem_recover_v1":
        ...
        break;
      case "gem_followup_v1":
        result = await handleCustomGemFollowup(cmd);
        break;
      case "list_tabs":
    ...
    } else if (cmd.action === CUSTOM_GEM_FOLLOWUP_ACTION) {
      result = customGemFollowupFailure(
        typeof cmd.requestId === "string" ? cmd.requestId : null,
        "handler",
        "handler_error",
        "The Custom Gem follow-up handler failed safely"
      );
    } else {
...
function customGemRevisionResultBase(requestId, overrides = {}) {
  const gemUrl = typeof overrides.gemUrl === "string" ? overrides.gemUrl : null;
  const currentUrl = typeof overrides.currentUrl === "string" ? overrides.currentUrl : null;
  const conversationUrl = typeof overrides.conversationUrl === "string" ? overrides.conversationUrl : null;
  const gemId = typeof overrides.gemId === "string" ? overrides.gemId : null;
  const normalizedRequestId = isUuid(requestId) ? requestId : null;
  return {
    version: CUSTOM_GEM_REVISION_RESULT_VERSION,
    ...
    gemId,
    gemUrl,
    currentUrl: conversationUrl || currentUrl,
    conversationUrl: conversationUrl || currentUrl
  };
}
...
function rebindCustomGemRevisionResultTarget(result, request) {
  return {
    ...result,
    gemId: request?.gemId || null,
    gemUrl: request?.gemUrl || null,
    currentUrl: request?.conversationUrl || null,
    conversationUrl: request?.conversationUrl || null,
    tabId: request?.tabId ?? result?.tabId ?? null
  };
}
function customGemFailure(requestId, stage, code, message, context = {}) {
  return customGemResultBase(requestId, {
    ...context,
    state: "failed",
    stage,
    error: { code, message }
...
function customGemFollowupFailure(requestId, stage, code, message, context = {}) {
  return customGemRevisionResultBase(requestId, {
    ...context,
    state: "failed",
    stage,
    error: { code, message }
...
function customGemAmbiguous(requestId, stage, code, message, context = {}) {
  return customGemResultBase(requestId, {
    ...context,
    state: "ambiguous",
    stage,
...
function customGemFollowupAmbiguous(requestId, stage, code, message, context = {}) {
  return customGemRevisionResultBase(requestId, {
    ...context,
    state: "ambiguous",
    stage,
...
function customGemPageDetails(payload) {
  ...
  if (Array.isArray(payload.controlLabels)) {
    const controlLabels = payload.controlLabels.filter((value) => typeof value === "string").map((value) => value.slice(0, 120)).slice(0, 8);
    if (controlLabels.length > 0) details.controlLabels = controlLabels;
  }
  return Object.keys(details).length > 0 ? details : void 0;
}
function customGemErrorCode(error, fallback = "browser_error") {
  const code = error?.code;
  return typeof code === "string" && /^[a-z0-9_]{1,80}$/.test(code) ? code : fallback;
}
function customGemErrorMessage(code) {
  const messages = {
    invalid_tab: "The explicit tabId does not exist",
    tab_create_failed: "The inactive Custom Gem tab could not be created",
    tab_navigation_failed: "The explicit tab could not be navigated to the exact Custom Gem URL",
    gem_url_mismatch: "The target tab did not remain on the exact Custom Gem URL",
    page_not_ready: "The exact Custom Gem page did not expose the required stable controls",
    page_script_unavailable: "The exact Custom Gem page could not be inspected safely",
    ...
    followup_attestation_rejected: "Browser Control did not authenticate this one-time follow-up receipt",
    ...
    durable_request_completed: "This request already completed and its no-resend tombstone remains durable",
    durable_request_conflict: "This requestId was already bound to different submit metadata",
    durable_state_write_failed: "The durable exactly-once submit state could not be written safely",
    preexisting_attachments: "The composer already contained attachments",
    composer_missing: "A stable Custom Gem composer was not found",
    composer_not_empty: "The Custom Gem composer was not empty before this request",
    composer_message_mismatch: "The composer did not contain exactly the supplied message",
    send_button_missing: "A unique enabled Custom Gem Send button was not found",
    response_selector_missing: "A stable assistant response selector was not found",
    response_timeout: "No new complete stable attributable assistant response arrived before timeout",
    no_new_assistant_response: "No new attributable assistant response was observed",
    message_attribution_failed: "The submitted message could not be matched to a new user message",
    multiple_new_assistant_responses: "More than one new assistant response was observed",
    response_not_stable: "The new assistant response did not become stable",
    response_too_large: "The assistant response exceeded the bounded result size",
    deadline_exceeded: "The bounded Custom Gem transaction deadline expired",
    browser_disconnected_after_submit: "The browser became unavailable after Send; delivery is ambiguous",
    mqtt_disconnected_after_submit: "MQTT disconnected after Send; delivery is ambiguous",
    ambiguous_after_submit: "The transaction outcome is ambiguous after Send",
    tab_locked: "The explicit tab is already locked by another Custom Gem request",
    tab_quarantined: "The explicit tab is quarantined after an ambiguous Custom Gem submission"
  };
  return messages[code] || "The Custom Gem transaction failed safely";
}
function customGemRuntimeDetail(error) {
  const message = error instanceof Error ? error.message : String(error || "unknown browser error");
  return message.replace(/[\r\n\t]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 500);
}
function customGemBrowserError(code) {
  const error = new Error(code);
  error.code = code;
  return error;
}
...
function withCustomGemDurableLock(operation) {
  const task = customGemDurableQueue.then(operation, operation);
  customGemDurableQueue = task.catch(() => void 0);
  return task;
}
async function readCustomGemDurableRequests() {
  const stored = await chrome.storage.local.get(CUSTOM_GEM_DURABLE_REQUESTS_KEY);
  const value = stored[CUSTOM_GEM_DURABLE_REQUESTS_KEY];
  return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}
async function writeCustomGemDurableRequests(value) {
...
async function customGemFollowupFingerprint(request) {
  return sha256Hex(new TextEncoder().encode(JSON.stringify({
    requestId: request.requestId,
    tabId: request.tabId,
    gemId: request.gemId,
...
function compactCustomGemDurableRequests(current) {
  const completedWithResults = Object.entries(current).filter(([, value]) => value?.status === "completed" && value.result).sort(([, left], [, right]) => Date.parse(right.updatedAt) - Date.parse(left.updatedAt));
  for (const [requestId, value] of completedWithResults.slice(CUSTOM_GEM_REPLAY_LIMIT)) {
    current[requestId] = {
      fingerprint: value.fingerprint,
      status: "completed_tombstone",
      updatedAt: value.updatedAt
    };
  }
  return current;
}
async function reserveCustomGemDurableRequest(request, fingerprint) {
  return withCustomGemDurableLock(async () => {
    const current = await readCustomGemDurableRequests();
    const existing = current[request.requestId];
    if (existing) {
      if (existing.fingerprint !== fingerprint) return { state: "conflict" };
      if (existing.status === "completed" && existing.result) {
        return { state: "completed", result: existing.result };
      }
      if (existing.status === "completed_tombstone") return { state: "completed_tombstone" };
      return { state: "incomplete" };
    }
    if (Object.keys(current).length >= CUSTOM_GEM_DURABLE_REQUEST_LIMIT) {
      throw new Error("Custom Gem durable request capacity is exhausted.");
    }
    ...
    await writeCustomGemDurableRequests(compactCustomGemDurableRequests(current));
    return { state: "reserved", fingerprint };
  });
}
async function completeCustomGemDurableRequest(requestId, fingerprint, result) {
  await withCustomGemDurableLock(async () => {
    const current = await readCustomGemDurableRequests();
    const existing = current[requestId];
    if (!existing || existing.fingerprint !== fingerprint || existing.status !== "started") {
      throw new Error("Custom Gem durable request reservation changed before completion.");
    }
    current[requestId] = {
      fingerprint,
      status: "completed",
...
function verifyTrustedCustomGemFollowupReceipt(request) {
  const payload = {
    type: "CONSUME_CUSTOM_GEM_FOLLOWUP_ATTESTATION_V1",
    browserControlExtensionId: request.authorizationReceipt.browserControlExtensionId,
    commandId: request.authorizationReceipt.commandId,
    receiptId: request.authorizationReceipt.receiptId,
    requestId: request.requestId,
    gemId: request.gemId,
    gemUrl: request.gemUrl,
    currentUrl: request.currentUrl,
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(customGemBrowserError("followup_attestation_rejected")), 5e3);
    chrome.runtime.sendMessage(request.authorizationReceipt.browserControlExtensionId, payload, (response) => {
      clearTimeout(timer);
      if (chrome.runtime.lastError || response?.ok !== true || response?.result?.consumed !== true || response.result.receiptId !== request.authorizationReceipt.receiptId || response.result.browserControlExtensionId !== request.authorizationReceipt.browserControlExtensionId || response.result.gemId !== request.gemId || response.result.gemUrl !== request.gemUrl || response.result.currentUrl !== request.currentUrl || response.result.conversationUrl !== request.conversationUrl) {
        const error = customGemBrowserError("followup_attestation_rejected");
        error.detail = String(
          chrome.runtime.lastError?.message || response?.error || "attestation response was invalid"
        ).replace(/[\r\n\t]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 300);
        reject(error);
...
function executeCustomGemScript(tabId, func, args, deadlineAt, world = "ISOLATED") {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true;
      reject(customGemBrowserError("deadline_exceeded"));
    }, remainingMs);
...
async function resolveCustomGemTargetTab(request) {
  ...
  try {
    ...
    if (customGemErrorCode(error) === "tab_create_failed") throw error;
    throw customGemBrowserError("tab_create_failed");
  }
}
async function navigateAndVerifyCustomGemTab(tabId, gemUrl, deadlineAt) {
  ...
  while (Date.now() < deadlineAt) {
    ...
    if (current?.url && current.url !== gemUrl && Date.now() - startedAt > 5e3) {
      throw customGemBrowserError("gem_url_mismatch");
    }
    await new Promise((resolve) => setTimeout(resolve, 250));
  }
  throw customGemBrowserError("deadline_exceeded");
}
function customGemPreflightPage(expectedUrl) {
  if (location.href !== expectedUrl) return { ready: false, errorCode: "gem_url_mismatch" };
  ...
  const findVisible = (root2, selectors, predicate) => {
    const result = [];
    for (const selector of selectors) {
      for (const element of root2.querySelectorAll(selector)) {
        if (isVisible(element) && (!predicate || predicate(element))) result.push(element);
      }
    }
  ...
  const composerCandidates = findVisible(document, composerSelectors);
  if (composerCandidates.length !== 1) {
    return { ready: false, errorCode: "composer_missing", candidateCount: composerCandidates.length };
  }
  const input = composerCandidates[0];
  let root = input;
  for (let depth = 0; depth < 10 && root; depth += 1) {
    if (root.matches?.('form, [role="form"], [data-testid*="composer" i], [data-test-id*="composer" i]')) break;
    root = root.parentElement;
  }
  root = root || input.parentElement || document.body;
  ...
  const isUploadControl = (element) => {
    const label = labelOf(element);
    return /(upload|attach|add file)/i.test(label) && !/(download|remove|delete)/i.test(label);
  };
  const uploadCandidates = findVisible(root, uploadSelectors, isUploadControl);
  if (uploadCandidates.length !== 1) {
    return {
      ready: false,
      errorCode: "upload_surface_missing",
  ...
  const isSendControl = (element) => {
    const label = labelOf(element);
    return /\bsend\b/i.test(label) && !/(feedback|invite|share)/i.test(label);
  };
  const sendCandidates = findVisible(root, sendSelectors, isSendControl);
  if (sendCandidates.length !== 1) {
    return { ready: false, errorCode: "send_button_missing", candidateCount: sendCandidates.length };
  }
  const responseSelector = responseSelectors.find(
    (selector) => Array.from(document.querySelectorAll(selector)).some(isVisible)
  ) || "model-response";
  return {
    ready: true,
    composerSelector: composerSelectors.find((selector) => input.matches(selector)) || composerSelectors[0],
    uploadSelector: uploadSelectors.find((selector) => uploadCandidates[0].matches(selector)) || uploadSelectors[0],
    sendSelector: sendSelectors.find((selector) => sendCandidates[0].matches(selector)) || sendSelectors[0],
    responseSelector,
    messageSelectorSupport: messageSelectors.some((selector) => document.querySelector(selector))
  };
}
async function preflightCustomGemPage(tabId, gemUrl, deadlineAt) {
  const retryUntil = Math.min(deadlineAt, Date.now() + 2e4);
  ...
  while (Date.now() < retryUntil) {
    try {
      ...
      if (customGemErrorCode(error) === "deadline_exceeded") throw error;
    }
    await new Promise((resolve) => setTimeout(resolve, 250));
  }
  return lastPayload || { ready: false, errorCode: "page_script_unavailable" };
}
async function customGemAttachPage(payload) {
  if (location.href !== payload?.expectedUrl) return { ok: false, errorCode: "gem_url_mismatch" };
  ...
  const inputCandidates = unique(composerSelectors.flatMap((selector) => Array.from(document.querySelectorAll(selector)).filter(isVisible)));
  if (inputCandidates.length !== 1) return { ok: false, errorCode: "composer_missing", candidateCount: inputCandidates.length };
  ...
  const uploadCandidates = unique(uploadSelectors.flatMap((selector) => Array.from(root.querySelectorAll(selector)).filter(isVisible)));
  if (uploadCandidates.length !== 1) return { ok: false, errorCode: "upload_surface_missing", candidateCount: uploadCandidates.length };
  const markerSelectors = [
    '[data-testid*="attachment" i]',
    '[data-test-id*="attachment" i]',
    '[data-testid*="file-chip" i]',
    '[data-test-id*="file-chip" i]',
    '[role="listitem"][aria-label*="file" i]',
    '[role="listitem"][aria-label*="attachment" i]'
  ];
  const attachmentNodes = () => {
    const candidates = unique(markerSelectors.flatMap((selector) => Array.from(root.querySelectorAll(selector)).filter(isVisible)));
    return candidates.filter((candidate) => !candidates.some((other) => other !== candidate && other.contains(candidate)));
  };
  const verifyAttachmentNodes = () => {
    const nodes = attachmentNodes();
    if (nodes.length === 0) return { ok: false, errorCode: "attachment_markers_missing", attachmentCount: 0 };
    if (nodes.length !== sources.length) return { ok: false, errorCode: "attachment_count_mismatch", attachmentCount: nodes.length };
    for (let index = 0; index < sources.length; index += 1) {
      if (!labelOf(nodes[index]).includes(String(sources[index].filename).toLowerCase())) {
        return { ok: false, errorCode: "attachment_names_unverified", attachmentCount: nodes.length };
      }
    }
    return { ok: true, attachmentCount: nodes.length };
  };
  const existingMarkers = attachmentNodes();
  if (existingMarkers.length > 0) return { ok: false, errorCode: "preexisting_attachments", attachmentCount: existingMarkers.length };
  const fileInputs = () => {
    const scoped = Array.from(root.querySelectorAll('input[type="file"]'));
    if (scoped.length > 0) return scoped;
    return Array.from(document.querySelectorAll('input[type="file"]'));
  };
  const existingInputs = fileInputs();
  if (existingInputs.some((fileInput2) => fileInput2.files?.length > 0)) {
    return { ok: false, errorCode: "preexisting_attachments", inputFileCount: existingInputs[0]?.files?.length || 0 };
  }
  if (existingInputs.length > 1) {
    return { ok: false, errorCode: "file_input_missing", inputFileCount: existingInputs.length };
  }
  let fileInput = existingInputs[0] || null;
  let capturedFileInput = fileInput;
  const observedControlLabels = [];
  if (!fileInput) {
    const originalFileInputClick = window.HTMLInputElement.prototype.click;
    const originalShowPicker = window.HTMLInputElement.prototype.showPicker;
    window.HTMLInputElement.prototype.click = function(...argumentsList) {
      if (this.type === "file" && !this.disabled) {
        capturedFileInput = this;
        return;
      }
      return originalFileInputClick.apply(this, argumentsList);
    };
    ...
    try {
      uploadCandidates[0].click();
      let menuClicked = false;
      for (let attempt = 0; attempt < 50; attempt += 1) {
        const candidates = unique([
          ...capturedFileInput ? [capturedFileInput] : [],
          ...fileInputs()
        ]).filter((candidate) => !candidate.disabled);
        if (candidates.length === 1) {
          fileInput = candidates[0];
          break;
        }
        if (candidates.length > 1) {
          return { ok: false, errorCode: "file_input_missing", inputFileCount: candidates.length };
        }
        const exactUploadButton = document.querySelector('[data-test-id="local-images-files-uploader-button"]');
        if (!menuClicked && exactUploadButton && isVisible(exactUploadButton)) {
          observedControlLabels.splice(0, observedControlLabels.length, labelOf(exactUploadButton));
          exactUploadButton.click();
          menuClicked = true;
          await new Promise((resolve) => setTimeout(resolve, 100));
          continue;
        }
        const menuCandidates = unique(Array.from(document.querySelectorAll('button[role^="menuitem"], button[role="option"], button[aria-label], [role="button"][aria-label]'))).filter(isVisible).filter((candidate) => candidate !== uploadCandidates[0]).filter((candidate) => /upload|attach|files?|computer|device/i.test(labelOf(candidate)) && !/download/i.test(labelOf(candidate)));
        observedControlLabels.splice(0, observedControlLabels.length, ...unique(menuCandidates.map(labelOf)).slice(0, 8));
        const preferredMenuCandidates = menuCandidates.filter((candidate) => /upload(?: files?| from)|choose files?|files? from|computer|device/i.test(labelOf(candidate)));
        if (!menuClicked && preferredMenuCandidates.length === 1) {
          preferredMenuCandidates[0].click();
          menuClicked = true;
        }
        await new Promise((resolve) => setTimeout(resolve, 100));
      }
    } finally {
      window.HTMLInputElement.prototype.click = originalFileInputClick;
      if (typeof originalShowPicker === "function") {
        window.HTMLInputElement.prototype.showPicker = originalShowPicker;
      }
    }
  }
  if (!fileInput) return { ok: false, errorCode: "file_input_missing", controlLabels: observedControlLabels };
  if (fileInput.multiple !== true) return { ok: false, errorCode: "file_input_missing", inputFileCount: 1 };
  if (typeof DataTransfer !== "function" || typeof File !== "function") {
    return { ok: false, errorCode: "file_input_missing" };
  }
  try {
    const transfer = new DataTransfer();
    for (const source of sources) {
      const binary = atob(String(source.base64 || ""));
      const bytes = new Uint8Array(binary.length);
      for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
      transfer.items.add(new File([bytes], source.filename, { type: source.mimeType }));
    }
  ...
  if (fileInput.files?.length !== sources.length) {
    return { ok: false, errorCode: "attachment_count_mismatch", inputFileCount: fileInput.files?.length || 0 };
  }
  for (let index = 0; index < sources.length; index += 1) {
    const file = fileInput.files[index];
    if (!file || file.name !== sources[index].filename || file.type !== sources[index].mimeType || file.size !== sources[index].bytes) {
      return { ok: false, errorCode: "attachment_count_mismatch", inputFileCount: fileInput.files?.length || 0 };
    }
  }
  let lastCount = 0;
  for (let attempt = 0; attempt < 50; attempt += 1) {
    const verified = verifyAttachmentNodes();
    if (verified.ok && verified.attachmentCount === sources.length && lastCount === verified.attachmentCount) {
      return { ok: true, attachmentCount: verified.attachmentCount, inputFileCount: fileInput.files.length };
    }
    lastCount = verified.attachmentCount || 0;
    await new Promise((resolve) => setTimeout(resolve, 120));
  }
  const finalState = verifyAttachmentNodes();
  return {
    ok: false,
    errorCode: finalState.errorCode || "attachment_count_mismatch",
    attachmentCount: finalState.attachmentCount || 0,
    inputFileCount: fileInput.files?.length || 0
  };
}
async function attachCustomGemSources(tabId, gemUrl, sources, deadlineAt) {
  ...
  return results?.[0]?.result || { ok: false, errorCode: "page_script_unavailable" };
}
async function customGemSubmitPage(payload) {
  if (location.href !== payload?.expectedUrl) return { state: "failed", errorCode: "gem_url_mismatch", clicked: false };
  ...
  const composerCandidates = unique(composerSelectors.flatMap((selector) => Array.from(document.querySelectorAll(selector)).filter(isVisible)));
  if (composerCandidates.length !== 1) return { state: "failed", errorCode: "composer_missing", candidateCount: composerCandidates.length, clicked: false };
  const input = composerCandidates[0];
  let root = input;
  for (let depth = 0; depth < 10 && root; depth += 1) {
    if (root.matches?.('form, [role="form"], [data-testid*="composer" i], [data-test-id*="composer" i]')) break;
  ...
  root = root || input.parentElement || document.body;
  const readComposerText = () => String(input.value ?? input.textContent ?? "").replace(/\u00a0/g, " ");
  if (readComposerText() !== "") return { state: "failed", errorCode: "composer_not_empty", clicked: false };
  const markerSelectors = [
    '[data-testid*="attachment" i]',
    '[data-test-id*="attachment" i]',
    '[data-testid*="file-chip" i]',
  ...
  const attachmentNodes = () => {
    const exactMediaTiles = unique(
      Array.from(document.querySelectorAll("gem-media-attachment.gem-attachment-tile")).filter(isVisible)
    );
    if (exactMediaTiles.length > 0) return exactMediaTiles;
    const candidates = unique(markerSelectors.flatMap((selector) => Array.from(root.querySelectorAll(selector)).filter(isVisible)));
    return candidates.filter((candidate) => !candidates.some((other) => other !== candidate && other.contains(candidate)));
  };
  const attachmentState = () => {
    ...
    if (exactMediaTiles) {
      return {
        ok: true,
        attachmentCount: nodes.length,
        attachmentVerification: "trusted_sequential_tiles"
      };
    }
    for (let index = 0; index < expectedSources.length; index += 1) {
      if (!labelOf(nodes[index]).includes(String(expectedSources[index].filename).toLowerCase())) {
        return { ok: false, errorCode: "attachment_names_unverified", attachmentCount: nodes.length };
      }
    }
  ...
  const messageSelectors = ["user-query", '[data-test-id="user-query"]', '[data-testid="user-query"]'];
  const messageNodes = () => unique(messageSelectors.flatMap((selector) => Array.from(document.querySelectorAll(selector)).filter(isVisible)));
  const baselineResponses = responseNodes();
  const baselineMessages = messageNodes();
  let messageProofRequired = baselineMessages.length > 0;
  let messageProof = !messageProofRequired;
...
\`\`\``);
  ...
  const writeComposerText = () => {
    ...
    if (!inserted) input.textContent = message;
  ...
  writeComposerText();
  if (readComposerText() !== message) return { state: "failed", errorCode: "composer_message_mismatch", clicked: false };
  if (Date.now() >= deadlineAt) return { state: "failed", errorCode: "deadline_exceeded", clicked: false };
  const normalizeSendSignal = (value) => String(value || "").replace(/\s+/g, " ").trim().toLowerCase();
  const isSendCandidate = (element) => {
    const ariaLabel = normalizeSendSignal(element.getAttribute("aria-label"));
    const text = normalizeSendSignal(element.textContent);
    const label = ariaLabel || text;
    const hasExactLabel = /^(?:send message|send|ส่งข้อความ|ส่ง)$/iu.test(label);
    const icon = element.querySelector("mat-icon,[data-icon]");
    const iconName = normalizeSendSignal(
      icon?.getAttribute("fonticon") || icon?.getAttribute("data-icon") || icon?.textContent
    );
    const hasSendIcon = /^(?:send|send_message|send-message)$/i.test(iconName);
    const classes = new Set(normalizeSendSignal(element.className).split(/\s+/).filter(Boolean));
    const hasSendClass = classes.has("send-button");
    if (ariaLabel && !hasExactLabel) return false;
    if (text && !ariaLabel && !hasExactLabel && !hasSendIcon && !hasSendClass) return false;
    return hasExactLabel || hasSendIcon || hasSendClass;
  };
  const getSendCandidates = () => unique(
    Array.from(root.querySelectorAll('button,[role="button"]')).filter(isVisible)
  ...
  while (Date.now() < sendReadyDeadline) {
    ...
    const enabled = candidate && !candidate.disabled && String(candidate.getAttribute("aria-disabled") || "").toLowerCase() !== "true";
    const currentAttachments = attachmentState();
    if (readComposerText() !== message || !currentAttachments.ok) {
      return { state: "failed", errorCode: "composer_message_mismatch", clicked: false };
    }
    if (enabled) {
  ...
  if (!sendCandidate) {
    return { state: "failed", errorCode: "send_button_missing", candidateCount: lastCandidateCount, clicked: false };
  }
  if (payload.prepareOnly === true) {
    if (typeof payload.attributionToken !== "string" || !/^[0-9a-f-]{36}$/.test(payload.attributionToken)) {
      return { state: "failed", errorCode: "message_attribution_failed", clicked: false };
    }
    for (const node of [...baselineResponses, ...baselineMessages]) {
      node.setAttribute("data-custom-gem-baseline", payload.attributionToken);
    }
    return {
      state: "prepared",
      baselineResponseCount: baselineResponses.length,
  ...
  while (Date.now() < deadlineAt) {
    ...
    if (!messageProofRequired && currentMessages.length > 0) messageProofRequired = true;
    if (messageProofRequired && !messageProof) {
      const newMessages = currentMessages.filter((node) => !baselineMessages.includes(node));
      if (newMessages.length > 1) {
        return { state: "ambiguous", errorCode: "message_attribution_failed", messageNodeCount: newMessages.length, clicked };
      }
      if (newMessages.length === 1) {
        if (!messageText(newMessages[0]).includes(message)) {
          return { state: "ambiguous", errorCode: "message_attribution_failed", messageNodeCount: newMessages.length, clicked };
        }
        messageProof = true;
      } else {
    ...
    const newResponses = currentResponses.filter((node) => !baselineResponses.includes(node));
    if (newResponses.length > 1) {
      return { state: "ambiguous", errorCode: "multiple_new_assistant_responses", responseNodeCount: newResponses.length, clicked };
    }
    if (newResponses.length === 1) {
      ...
      if (text.trim() && responseComplete(node)) {
        if (candidateNode === node && candidateText === text) {
          await sleep(800);
          const stableResponses = responseNodes().filter((item) => !baselineResponses.includes(item));
          ...
          if (stableResponses.length === 1 && stableResponses[0] === node && stableText === text && responseComplete(node)) {
            ...
            if (responseUtf8Bytes > 64 * 1024) {
              return { state: "failed", errorCode: "response_too_large", responseUtf8Bytes, clicked };
            }
            return {
              state: "succeeded",
              response: text,
  ...
  return {
    state: "ambiguous",
    errorCode: messageProofRequired && !messageProof ? "message_attribution_failed" : "response_timeout",
    clicked
  };
}
...
\`\`\``);
  ...
  const messageNodes = () => unique([
    ...
    ...document.querySelectorAll('[data-testid="user-query"]')
  ].filter(isVisible));
  const messageText = (node) => {
    const lines = Array.from(node.querySelectorAll(".query-text-line"));
    if (lines.length > 0) {
  ...
  const cleanup = () => {
    for (const node of document.querySelectorAll("[data-custom-gem-baseline]")) {
      if (node.getAttribute("data-custom-gem-baseline") === attributionToken) {
        node.removeAttribute("data-custom-gem-baseline");
      }
    }
  };
  const finish = (result) => {
...
function cleanupCustomGemAttributionPage(attributionToken) {
  for (const node of document.querySelectorAll("[data-custom-gem-baseline]")) {
    if (node.getAttribute("data-custom-gem-baseline") === attributionToken) {
      node.removeAttribute("data-custom-gem-baseline");
    }
...
async function runTrustedCustomGemSubmit(tabId, request, responseSelector, deadlineAt) {
  ...
  try {
    ...
    await chrome.scripting.executeScript({
      ...
      args: [attributionToken]
    }).catch(() => void 0);
  }
}
function requestTrustedCustomGemFollowupSend(request) {
  const payload = {
    type: "TRUSTED_CUSTOM_GEM_FOLLOWUP_SEND_V1",
    browserControlExtensionId: request.authorizationReceipt.browserControlExtensionId,
    receiptId: request.authorizationReceipt.receiptId,
    requestId: request.requestId,
    gemId: request.gemId,
  ...
  return new Promise((resolve, reject) => {
    ...
    chrome.runtime.sendMessage(request.authorizationReceipt.browserControlExtensionId, payload, (response) => {
      clearTimeout(timer);
      if (chrome.runtime.lastError || response?.ok !== true || response?.result?.clicked !== true || response.result.receiptId !== request.authorizationReceipt.receiptId || response.result.browserControlExtensionId !== request.authorizationReceipt.browserControlExtensionId || response.result.gemId !== request.gemId || response.result.gemUrl !== request.gemUrl || response.result.currentUrl !== request.currentUrl || response.result.conversationUrl !== request.conversationUrl) {
        const error = customGemBrowserError("trusted_send_rejected");
        error.detail = String(
          chrome.runtime.lastError?.message || response?.error || "trusted send response was invalid"
        ).replace(/[\r\n\t]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 300);
...
async function customGemFollowupPreparePage(payload) {
  if (location.href !== payload?.expectedUrl) return { state: "failed", errorCode: "gem_url_mismatch", clicked: false };
  const message = String(payload?.message || "");
  const deadlineAt = Number(payload?.deadlineAt || 0);
  const sendStableMs = Number(payload?.sendStableMs || 2e3);
  const sendReadyWindowMs = Number(payload?.sendReadyWindowMs || 12e3);
  const attributionToken = String(payload?.attributionToken || "");
  const sleep = (milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds));
  const isVisible = (element) => {
    if (!element) return false;
    const rect = element.getBoundingClientRect();
  ...
  if (readComposerText() !== "") return { state: "failed", errorCode: "composer_not_empty", clicked: false };
  const markerSelectors = [
    "gem-media-attachment.gem-attachment-tile",
    '[data-testid*="attachment" i]',
    '[data-test-id*="attachment" i]',
    '[data-testid*="file-chip" i]',
    '[data-test-id*="file-chip" i]',
  ...
  const attachmentNodes = unique(markerSelectors.flatMap((selector) => Array.from(root.querySelectorAll(selector)).filter(isVisible)));
  if (attachmentNodes.length > 0) {
    return { state: "failed", errorCode: "preexisting_attachments", attachmentCount: attachmentNodes.length, clicked: false };
  }
  const fileInputs = Array.from(document.querySelectorAll('input[type="file"]'));
  if (fileInputs.some((fileInput) => fileInput.files?.length > 0)) {
    return { state: "failed", errorCode: "preexisting_attachments", inputFileCount: fileInputs[0]?.files?.length || 0, clicked: false };
  }
  const responseSelectors = [
    "model-response",
    '[data-test-id="response"]',
  ...
  while (Date.now() < sendReadyDeadline) {
    ...
    const candidate = candidates.length === 1 ? candidates[0] : null;
    const enabled = candidate && !candidate.disabled && String(candidate.getAttribute("aria-disabled") || "").toLowerCase() !== "true";
    const currentAttachmentNodes = unique(markerSelectors.flatMap((selector) => Array.from(root.querySelectorAll(selector)).filter(isVisible)));
    if (readComposerText() !== message || currentAttachmentNodes.length > 0) {
      return { state: "failed", errorCode: "composer_message_mismatch", clicked: false };
    }
    if (enabled) {
      readySince = readySince || Date.now();
  ...
  for (const node of [...baselineResponses, ...baselineMessages]) {
    node.setAttribute("data-custom-gem-baseline", attributionToken);
  }
  return {
    state: "prepared",
    responseSelector,
...
async function runTrustedCustomGemFollowup(tabId, request, deadlineAt, onSendAttempt) {
  const attributionToken = request.requestId;
  const preparedResults = await executeCustomGemScript(
    tabId,
    customGemFollowupPreparePage,
    [{
      expectedUrl: request.conversationUrl,
      message: request.message,
      sendStableMs: CUSTOM_GEM_BASE_SEND_STABLE_MS,
      sendReadyWindowMs: CUSTOM_GEM_BASE_SEND_STABLE_MS + CUSTOM_GEM_SEND_READY_GRACE_MS,
      attributionToken,
      deadlineAt
    }],
    deadlineAt
  ...
  const prepared = preparedResults?.[0]?.result || null;
  if (!prepared || prepared.state !== "prepared") return prepared;
  const responseSelector = prepared.responseSelector || "model-response";
  try {
    onSendAttempt();
    await requestTrustedCustomGemFollowupSend(request);
    const responseResults = await executeCustomGemScript(
      tabId,
      awaitTrustedCustomGemResponsePage,
      [{
...
async function executeCustomGemStart(cmd, validatedRequest = null) {
  ...
  try {
    tabId = await resolveCustomGemTargetTab(request);
    const tabContext = { ...context, tabId };
    if (customGemQuarantinedTabs.has(tabId)) {
      return customGemFailure(request.requestId, "lock", "tab_quarantined", customGemErrorMessage("tab_quarantined"), tabContext);
    }
    const currentLock = customGemTabLocks.get(tabId);
    if (currentLock && currentLock !== request.requestId) {
      return customGemFailure(request.requestId, "lock", "tab_locked", customGemErrorMessage("tab_locked"), tabContext);
    }
    ...
    if (!preflight?.ready) {
      ...
      return customGemFailure(request.requestId, "preflight", code, customGemErrorMessage(code), {
        ...tabContext,
        details: customGemPageDetails(preflight)
      });
    }
    ...
    if (!attachmentResult?.ok) {
      ...
      return customGemFailure(request.requestId, "attachments", code, customGemErrorMessage(code), {
        ...tabContext,
        details: customGemPageDetails(attachmentResult)
      });
    }
    if (Date.now() >= deadlineAt) {
      return customGemFailure(request.requestId, "submit", "deadline_exceeded", customGemErrorMessage("deadline_exceeded"), tabContext);
    }
    submitAttempted = true;
    let submitResult;
    try {
      submitResult = await runCustomGemSubmit(tabId, request, preflight.responseSelector, deadlineAt);
    } catch {
      customGemQuarantinedTabs.set(tabId, request.requestId);
      return customGemAmbiguous(
        request.requestId,
        "submit",
        "browser_disconnected_after_submit",
        customGemErrorMessage("browser_disconnected_after_submit"),
        tabContext
      );
    }
    if (!connected) {
      ...
      return customGemAmbiguous(
        request.requestId,
        "submit",
        "mqtt_disconnected_after_submit",
        customGemErrorMessage("mqtt_disconnected_after_submit"),
        tabContext
      );
    }
    if (!submitResult || submitResult.state === "ambiguous") {
      customGemQuarantinedTabs.set(tabId, request.requestId);
      const code = submitResult?.errorCode || "ambiguous_after_submit";
      return customGemAmbiguous(request.requestId, "response", code, customGemErrorMessage(code), {
        ...tabContext,
        details: customGemPageDetails(submitResult)
      });
    }
    if (submitResult.state !== "succeeded") {
      const code = submitResult.errorCode || "browser_error";
      return customGemFailure(request.requestId, "submit", code, customGemErrorMessage(code), {
        ...tabContext,
        details: customGemPageDetails(submitResult)
      });
    }
    const rawResponse = typeof submitResult.response === "string" ? submitResult.response : "";
    const responseUtf8Bytes = utf8ByteLength(rawResponse);
    if (!rawResponse || responseUtf8Bytes > CUSTOM_GEM_MAX_RESPONSE_BYTES) {
      return customGemFailure(request.requestId, "response", "response_too_large", customGemErrorMessage("response_too_large"), tabContext);
    }
    return customGemResultBase(request.requestId, {
      ...tabContext,
      state: "succeeded",
      stage: "response_complete",
      conversationUrl: typeof submitResult.conversationUrl === "string" ? submitResult.conversationUrl : null,
      responseSha256: await sha256Hex(new TextEncoder().encode(rawResponse)),
      rawResponse,
      responseUtf8Bytes
    });
  } catch (error) {
    if (submitAttempted && tabId !== null) {
      customGemQuarantinedTabs.set(tabId, requestId);
      return customGemAmbiguous(
        requestId,
        "submit",
        "browser_disconnected_after_submit",
    ...
    return customGemFailure(requestId, code === "deadline_exceeded" ? "timeout" : "browser", code, customGemErrorMessage(code), {
      ...context,
      tabId,
      details: { runtimeError: customGemRuntimeDetail(error) }
    });
  } finally {
    if (lockHeld && tabId !== null && customGemTabLocks.get(tabId) === requestId) {
      customGemTabLocks.delete(tabId);
    }
  }
}
...
async function executeCustomGemSubmit(cmd) {
  ...
  let validation;
  try {
    validation = await validateGemSubmitRequest(cmd);
  } catch {
    return customGemFailure(requestId, "validation", "validator_error", "The Custom Gem submit validator failed safely");
  }
  if (!validation.ok) {
    return customGemFailure(
      requestId,
  ...
  try {
    let tab;
    const tabReadyDeadline = Math.min(deadlineAt, Date.now() + 5e3);
    while (Date.now() < tabReadyDeadline) {
      try {
        tab = await chrome.tabs.get(request.tabId);
      } catch {
    ...
    if (tab?.status !== "complete") {
      return customGemFailure(request.requestId, "preflight", "page_not_ready", customGemErrorMessage("page_not_ready"), context);
    }
    if (customGemQuarantinedTabs.has(request.tabId)) {
      return customGemFailure(request.requestId, "lock", "tab_quarantined", customGemErrorMessage("tab_quarantined"), context);
    }
    const currentLock = customGemTabLocks.get(request.tabId);
    if (currentLock && currentLock !== request.requestId) {
      return customGemFailure(request.requestId, "lock", "tab_locked", customGemErrorMessage("tab_locked"), context);
    }
    ...
    if (Date.now() >= deadlineAt) {
      return customGemFailure(request.requestId, "submit", "deadline_exceeded", customGemErrorMessage("deadline_exceeded"), context);
    }
    try {
      await verifyTrustedCustomGemUploadReceipt(request);
    } catch (error) {
      return customGemFailure(
        request.requestId,
        "attestation",
        "upload_attestation_rejected",
        customGemErrorMessage("upload_attestation_rejected"),
        {
          ...context,
          details: {
            runtimeError: customGemRuntimeDetail(error?.detail || error)
          }
        }
      );
    }
    ...
    let submitResult;
    try {
      submitResult = await runTrustedCustomGemSubmit(request.tabId, request, preflight.responseSelector, deadlineAt);
    } catch (error) {
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      const code = customGemErrorCode(error, "browser_disconnected_after_submit");
      return customGemAmbiguous(
        request.requestId,
        "submit",
        code,
        ...
        {
          ...context,
          details: { runtimeError: customGemRuntimeDetail(error?.detail || error) }
        }
      );
    }
    if (!connected) {
    ...
    if (submitResult.state !== "succeeded") {
      const code = submitResult.errorCode || "browser_error";
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      return customGemAmbiguous(request.requestId, "submit", code, customGemErrorMessage(code), {
        ...context,
        details: customGemPageDetails(submitResult)
      });
    }
    ...
    if (submitAttempted) {
      customGemQuarantinedTabs.set(request.tabId, requestId);
      return customGemAmbiguous(
        requestId,
        "submit",
        "browser_disconnected_after_submit",
    ...
    if (lockHeld && customGemTabLocks.get(request.tabId) === requestId) {
      customGemTabLocks.delete(request.tabId);
    }
  }
}
...
async function handleCustomGemSubmit(cmd) {
  const requestId = typeof cmd?.requestId === "string" ? cmd.requestId : null;
  let validation;
  try {
    validation = await validateGemSubmitRequest(cmd);
  ...
  const task = (async () => {
    ...
    try {
      await completeCustomGemDurableRequest(requestId, reservation.fingerprint, result);
    } catch {
      return customGemAmbiguous(
        requestId,
        "durable_state",
  ...
  if (isUuid(requestId)) customGemSubmitInFlight.set(requestId, { fingerprint, task });
  try {
    return await task;
  } finally {
    if (isUuid(requestId) && customGemSubmitInFlight.get(requestId)?.task === task) {
      customGemSubmitInFlight.delete(requestId);
    }
  }
}
async function executeCustomGemFollowup(cmd, validatedRequest = null) {
  const requestId = typeof cmd?.requestId === "string" ? cmd.requestId : null;
  let validation;
  if (validatedRequest) {
    validation = { ok: true, request: validatedRequest };
    ...
    try {
      validation = await validateGemFollowupRequest(cmd);
    } catch {
      return customGemFollowupFailure(requestId, "validation", "validator_error", "The Custom Gem follow-up validator failed safely");
    }
  }
  if (!validation.ok) {
    return customGemFollowupFailure(
      requestId,
      validation.error.stage,
      validation.error.code,
      validation.error.message
  ...
  try {
    ...
    while (Date.now() < tabReadyDeadline) {
      try {
        ...
        return customGemFollowupFailure(request.requestId, "preflight", "invalid_tab", customGemErrorMessage("invalid_tab"), context);
      }
      if (tab?.url !== request.conversationUrl) {
        return customGemFollowupFailure(request.requestId, "preflight", "gem_url_mismatch", customGemErrorMessage("gem_url_mismatch"), context);
      }
      if (tab.status === "complete") break;
      await new Promise((resolve) => setTimeout(resolve, 100));
    }
    if (tab?.status !== "complete") {
      return customGemFollowupFailure(request.requestId, "preflight", "page_not_ready", customGemErrorMessage("page_not_ready"), context);
    }
    if (customGemQuarantinedTabs.has(request.tabId)) {
      return customGemFollowupFailure(request.requestId, "lock", "tab_quarantined", customGemErrorMessage("tab_quarantined"), context);
    }
    const currentLock = customGemTabLocks.get(request.tabId);
    if (currentLock && currentLock !== request.requestId) {
      return customGemFollowupFailure(request.requestId, "lock", "tab_locked", customGemErrorMessage("tab_locked"), context);
    }
    customGemTabLocks.set(request.tabId, request.requestId);
    lockHeld = true;
    if (Date.now() >= deadlineAt) {
      return customGemFollowupFailure(request.requestId, "submit", "deadline_exceeded", customGemErrorMessage("deadline_exceeded"), context);
    }
    try {
      await verifyTrustedCustomGemFollowupReceipt(request);
    } catch (error) {
      return customGemFollowupFailure(
        request.requestId,
        "attestation",
        "followup_attestation_rejected",
        customGemErrorMessage("followup_attestation_rejected"),
        {
          ...context,
          details: {
            runtimeError: customGemRuntimeDetail(error?.detail || error)
    ...
    let submitResult;
    try {
      submitResult = await runTrustedCustomGemFollowup(
        request.tabId,
        request,
        deadlineAt,
        () => {
      ...
      if (!submitAttempted) {
        const code2 = customGemErrorCode(error);
        return customGemFollowupFailure(
          request.requestId,
          code2 === "deadline_exceeded" ? "timeout" : "prepare",
          code2,
          customGemErrorMessage(code2),
          {
            ...context,
            details: { runtimeError: customGemRuntimeDetail(error?.detail || error) }
          }
      ...
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      const code = customGemErrorCode(error, "browser_disconnected_after_submit");
      return customGemFollowupAmbiguous(
        request.requestId,
        "submit",
        code,
        customGemErrorMessage(code),
    ...
    if (!submitResult || submitResult.state === "ambiguous") {
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      const code = submitResult?.errorCode || "ambiguous_after_submit";
      return customGemFollowupAmbiguous(request.requestId, "response", code, customGemErrorMessage(code), {
        ...context,
        details: customGemPageDetails(submitResult)
      });
    }
    if (submitResult.state !== "succeeded") {
      const code = submitResult.errorCode || "browser_error";
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      return customGemFollowupAmbiguous(request.requestId, "submit", code, customGemErrorMessage(code), {
        ...context,
        details: customGemPageDetails(submitResult)
      });
    }
    ...
    if (!rawResponse || responseUtf8Bytes > CUSTOM_GEM_MAX_RESPONSE_BYTES) {
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      return customGemFollowupAmbiguous(request.requestId, "response", "response_too_large", customGemErrorMessage("response_too_large"), context);
    }
    if (!hasRequiredTerminalMarkers(rawResponse, request.requiredTerminalMarkers)) {
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      return customGemFollowupAmbiguous(request.requestId, "response", "response_timeout", customGemErrorMessage("response_timeout"), context);
    }
    if (!isExactCustomGemConversationUrl(request.gemUrl, submitResult.conversationUrl)) {
      customGemQuarantinedTabs.set(request.tabId, request.requestId);
      return customGemFollowupAmbiguous(request.requestId, "response", "gem_url_mismatch", customGemErrorMessage("gem_url_mismatch"), context);
    }
    return {
      version: CUSTOM_GEM_REVISION_RESULT_VERSION,
      id: request.requestId,
    ...
    const code = customGemErrorCode(error);
    return customGemFollowupFailure(requestId, code === "deadline_exceeded" ? "timeout" : "browser", code, customGemErrorMessage(code), {
      ...context,
      details: { runtimeError: customGemRuntimeDetail(error) }
    });
  } finally {
...
async function handleCustomGemFollowup(cmd) {
  const requestId = typeof cmd?.requestId === "string" ? cmd.requestId : null;
  let validation;
  try {
    validation = await validateGemFollowupRequest(cmd);
  ...
  const fingerprint = await customGemFollowupFingerprint(request);
  const inFlight = isUuid(requestId) ? customGemFollowupInFlight.get(requestId) : null;
  if (inFlight) {
    if (inFlight.fingerprint !== fingerprint) {
      return customGemFollowupFailure(
        requestId,
  ...
  const task = (async () => {
    ...
    if (reservation.state === "completed") return rebindCustomGemRevisionResultTarget(reservation.result, request);
    if (reservation.state === "completed_tombstone") {
      return customGemFollowupAmbiguous(
        requestId,
        "durable_state",
    ...
    const result = await executeCustomGemFollowup(cmd, request);
    try {
      await completeCustomGemDurableRequest(requestId, reservation.fingerprint, result);
    } catch {
      return customGemFollowupAmbiguous(
  ...
  if (isUuid(requestId)) customGemFollowupInFlight.set(requestId, { fingerprint, task });
  try {
    return await task;
  } finally {
    if (isUuid(requestId) && customGemFollowupInFlight.get(requestId)?.task === task) {
      customGemFollowupInFlight.delete(requestId);
    }
  }
}
...
async function listFlowTabs() {
  ...
  return {
    success: true,
    tabs: tabs.filter((t) => isFlowUrl(t.url)).map((t) => ({
      id: t.id,
      title: t.title,
      url: t.url,
      active: t.active,
...
async function createFlowTab(url, projectId) {
  ...
  const targetUrl = typeof url === "string" && url.startsWith("http") ? url : normalizedProjectId ? flowProjectUrl(normalizedProjectId) : flowUrl();
  const tab = await chrome.tabs.create({ url: targetUrl });
      ...
      func: async () => {
        ...
        for (let i = 0; i < 12; i += 1) {
          ...
          for (const phrase of priorities) {
            target = candidates.find((el2) => {
              const label = normalize(el2.getAttribute("aria-label") || el2.textContent);
...
async function flowSelectTool(tabId, tool, projectId) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (toolName) => {
      ...
      const clickElement = async (el2) => {
        if (!el2) return;
        const overlay = el2.querySelector('[data-type="button-overlay"]');
        const target = overlay || el2;
      ...
      const closeImagePreviewIfNeeded = async () => {
        ...
        const exitButton = buttons.find((el2) => {
          const label = norm(el2.getAttribute("aria-label") || el2.textContent);
      ...
      const getToolControls = () => {
        const controls2 = Array.from(document.querySelectorAll('button[role="combobox"], button[aria-haspopup], button, [role="button"]')).filter(isVisible);
        return controls2.filter((el2) => {
          const txt = readComboboxLabel(el2);
          return knownTools.some((toolItem) => txt.includes(toolItem));
        });
      };
      const findToolCombobox = () => {
        const controls2 = getToolControls();
        return controls2[0] || null;
      };
      ...
      const pickFromMenuUsingCombobox = async (combobox) => {
        ...
        if (!controlsId) {
          ...
          for (const root of fallbackRoots) {
            ...
            const match = options.find((el2) => {
              const style = window.getComputedStyle(el2);
              if (style.display === "none" || style.visibility === "hidden") return false;
              const text = el2.getAttribute("aria-label") || el2.textContent;
      ...
      const tryOpenToolMenuAndPick = async () => {
        ...
        const buttons = Array.from(document.querySelectorAll('button, [role="button"]')).filter(isVisible);
        const addButton = buttons.find((el2) => {
          const label = norm(el2.getAttribute("aria-label") || el2.textContent);
...
async function flowCountImageTiles(tabId, projectId) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      const tiles = Array.from(document.querySelectorAll('button, [role="button"], [role="option"], li, div')).filter(isVisible).filter((el2) => {
        const rect = el2.getBoundingClientRect();
        if (rect.width < 28 || rect.height < 28 || rect.width > 280 || rect.height > 280) return false;
        return Boolean(el2.querySelector("img, canvas, video"));
      });
      const progressNodes = Array.from(document.querySelectorAll('span, div, p, [role="status"]')).filter(isVisible);
      const hasPercentProgress = progressNodes.some((el2) => /\b\d{1,3}%\b/.test(String(el2.textContent || "").trim()));
...
async function flowWaitForImageReady(tabId, projectId, baselineCount, timeoutMs = 9e4) {
...
async function flowClearSelectedIngredients(tabId, projectId) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const click = (el2) => {
        el2.dispatchEvent(new MouseEvent("pointerdown", { bubbles: true }));
        ...
        if (typeof el2.click === "function") el2.click();
      };
      const selectedTiles = Array.from(document.querySelectorAll('[aria-selected="true"], [aria-pressed="true"], [data-selected="true"]')).filter(isVisible).filter((el2) => Boolean(el2.querySelector("img, canvas, video")));
      let cleared = 0;
      ...
      const removeButtons = Array.from(document.querySelectorAll('button, [role="button"]')).filter(isVisible).filter((el2) => {
        const label = norm(el2.getAttribute("aria-label") || el2.textContent);
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const getOpenAssetRows = (controlsId) => {
        ...
        const scopedList = scopedDialog?.querySelector('[data-testid="virtuoso-item-list"]') || document.querySelector('[role="dialog"][data-state="open"] [data-testid="virtuoso-item-list"]');
      ...
      const isReferenceButton = (el2) => {
        if (!el2 || !isVisible(el2)) return false;
        const label = norm(el2.getAttribute("aria-label") || el2.textContent || "");
        if (!label) return false;
      ...
      const findReferenceDialogTrigger = () => {
        const candidates2 = Array.from(document.querySelectorAll('button[aria-haspopup="dialog"], [role="button"][aria-haspopup="dialog"], button, [role="button"]')).filter(isVisible);
        const direct = candidates2.find((el2) => isReferenceButton(el2));
        if (direct) return direct;
        const withRefIcon = candidates2.find((el2) => {
          const iconText = norm(Array.from(el2.querySelectorAll("i, span")).map((n) => n.textContent || "").join(" "));
          return iconText.includes("reference") || iconText.includes("ref");
        });
        return withRefIcon || null;
      };
      const findInboxAssetDialogTrigger = () => {
        const isAdd2Button = (el2) => {
          const iconText = norm(Array.from(el2.querySelectorAll("i, span")).map((n) => n.textContent || "").join(" "));
          const label = norm(el2.getAttribute("aria-label") || el2.textContent || "");
        ...
        for (const start of composerStarts) {
          ...
          for (let depth = 0; depth < 8 && node; depth += 1) {
            ...
            if (hasTextbox) {
              const candidates2 = Array.from(node.querySelectorAll('button[aria-haspopup="dialog"], [role="button"][aria-haspopup="dialog"]')).filter(isVisible);
              const add2 = candidates2.find((btn) => {
        ...
        return triggerCandidates.find((el2) => {
          const ariaControls = norm(el2.getAttribute("aria-controls") || "");
          return ariaControls.startsWith("radix-") && isAdd2Button(el2);
        }) || null;
      };
      const ensureAssetRowsReady = async () => {
        ...
        for (let attempt = 0; attempt < 8; attempt += 1) {
          const rows2 = getOpenAssetRows(controlsId);
          if (rows2.length > 0) return { rows: rows2, controlsId, openedWithTrigger, openedBy };
          ...
          if (candidate) {
            ...
            openedWithTrigger = String(candidate.el.getAttribute("aria-expanded") || "").toLowerCase() === "true" || String(candidate.el.getAttribute("data-state") || "").toLowerCase() === "open";
      ...
      const pickLatestRow = (rows2) => {
        if (!rows2.length) return null;
        const withIndex = rows2.map((row) => ({ row, idx: Number.parseInt(String(row.getAttribute("data-index") || ""), 10) })).filter((entry) => Number.isFinite(entry.idx));
      ...
      const candidates = Array.from(document.querySelectorAll('button, [role="button"], [role="option"], li, div')).filter(isVisible).map((el2) => {
        const rect = el2.getBoundingClientRect();
        const label = norm(el2.getAttribute("aria-label") || el2.textContent);
        const img = el2.querySelector("img, canvas, video");
        ...
        return { el: el2, rect, label, likelyThumb, blocked };
      }).filter((x2) => x2.likelyThumb && !x2.blocked);
      candidates.sort((a, b) => {
...
async function flowSubmitPrompt(tabId, prompt, projectId) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (message) => {
      ...
      const norm = (txt) => String(txt || "").replace(/\s+/g, " ").trim().toLowerCase();
      const compact = (txt) => norm(txt).replace(/[^a-z0-9]+/g, "");
      const sanitizeEditableText = (text2) => {
        const cleaned = String(text2 || "").replace(/\u200B|\u200C|\u200D|\uFEFF/g, "").replace(/\s+/g, " ").trim();
        return cleaned.replace(/^what do you want to create\?\s*/i, "").replace(/^start creating\s*/i, "").trim();
      };
      const readSlateStringText = (el2) => {
        if (!el2 || typeof el2.querySelectorAll !== "function") return "";
        const leaves = Array.from(el2.querySelectorAll('[data-slate-string="true"]'));
        if (leaves.length === 0) return "";
        return sanitizeEditableText(leaves.map((leaf) => String(leaf.textContent || "")).join(" "));
      };
      const readInputValue = (el2) => {
        if (!el2) return "";
        if (el2.getAttribute("contenteditable") === "true" || el2.tagName === "DIV") {
          const slateText = readSlateStringText(el2);
          if (slateText) return slateText;
          return sanitizeEditableText(el2.textContent || "");
        }
        return String(el2.value || "").trim();
      };
      const isEditImageContext = (el2) => {
        const context = el2?.closest('section, form, [role="dialog"], [data-radix-popper-content-wrapper], div');
        const text2 = norm(context?.textContent || "");
        return text2.includes("edit image") || text2.includes("annotations") || text2.includes("add to prompt");
      };
      ...
      let input = composerContainer ? Array.from(composerContainer.querySelectorAll('div[role="textbox"][contenteditable="true"][data-slate-editor="true"], [role="textbox"][contenteditable="true"], textarea')).find((el2) => isVisible(el2) && !isEditImageContext(el2)) || null : null;
      if (!input) {
        const fallbackInputs = Array.from(document.querySelectorAll('div[role="textbox"][contenteditable="true"][data-slate-editor="true"], [role="textbox"][contenteditable="true"], textarea')).filter((el2) => isVisible(el2) && !isEditImageContext(el2));
      ...
      const getVisiblePromptRequiredErrorCount = () => {
        ...
        return nodes.filter((el2) => {
          if (!isVisible(el2)) return false;
          const txt = norm(el2.textContent || "");
      ...
      const isSubmitButtonCandidate = (btn) => {
        ...
        const blocked = label.includes("download") || label.includes("edit image") || label.includes("add to prompt") || label.includes("add media") || label.includes("scenebuilder") || label.includes("upload") || label.includes("attach") || label === "add" || label === "+" || label === "add_2" || label === "exit";
      ...
      const getNearestSendButton = () => {
        ...
        for (const btn of buttons) {
          ...
          const dx2 = Math.abs(rect.left - inputRect.right);
          const dy2 = Math.abs(rect.top - inputRect.bottom);
          const score = dx2 + dy2;
          if (score < bestScore) {
            bestScore = score;
            winner = btn;
      ...
      const submitted = async (allowNoSignal = false) => {
        ...
        while (Date.now() - start < 3500) {
          ...
          const send2 = getSend();
          const disabled = Boolean(send2?.disabled) || String(send2?.getAttribute("aria-disabled") || "").toLowerCase() === "true";
      ...
      const tryKeyboardSubmit = async () => {
        ...
        for (const payload of keyVariants) {
          ...
          const ok2 = await submitted(false);
          if (ok2) return true;
        }
        return false;
      };
      ...
      if (send && !send.disabled) {
        ...
        const ok2 = await submitted(true);
        if (ok2) {
          return { success: true, method: "button" };
        }
        const keyboardFallback = await tryKeyboardSubmit();
        return keyboardFallback ? { success: true, method: "button+enter-fallback" } : { success: false, error: "Prompt not submitted after button click and keyboard fallback" };
      }
      const ok = await tryKeyboardSubmit();
      return ok ? { success: true, method: "enter" } : { success: false, error: "Prompt not submitted after keyboard submit attempts" };
    },
    args: [text]
  });
  const result = results[0]?.result;
    ...
    const results = await chrome.scripting.executeScript({
      ...
      func: async (cfg) => {
        ...
        const clickByText = async (container, wanted) => {
          ...
          const candidates = Array.from(container.querySelectorAll('[role="option"], [role="menuitem"], [role="menuitemcheckbox"], [role="menuitemradio"], li, button, [role="button"], label, span')).filter(isVisible);
          const match = candidates.find((el2) => {
            const raw = el2.getAttribute("aria-label") || el2.textContent;
        ...
        const findSettingsTrigger = () => {
          ...
          return settingsButtons.find((el2) => {
            const label = norm(el2.getAttribute("aria-label") || el2.textContent);
        ...
        const findSettingsPanel = () => {
          ...
          return panelRoots.find((el2) => {
            ...
            const combos = Array.from(el2.querySelectorAll('button[role="combobox"], [role="combobox"]')).filter(isVisible);
        ...
        const clickOutsidePanel = (panel2) => {
          if (!panel2) return;
          const rect = panel2.getBoundingClientRect();
          const x2 = Math.max(2, Math.floor(rect.left - 12));
          const y = Math.max(2, Math.floor(rect.top - 12));
          const target = document.elementFromPoint(x2, y) || document.body;
          const events = ["pointerdown", "mousedown", "mouseup", "click", "pointerup"];
          for (const type of events) {
            target.dispatchEvent(new MouseEvent(type, { bubbles: true, cancelable: true, clientX: x2, clientY: y }));
          }
        };
        const closeSettingsPanel = async () => {
          for (let attempt = 0; attempt < 3; attempt += 1) {
            let panel2 = findSettingsPanel();
            if (!panel2) return true;
            dispatchEscape(document.activeElement || document.body);
            dispatchEscape(document);
            await sleep(120);
            panel2 = findSettingsPanel();
            if (!panel2) return true;
            clickOutsidePanel(panel2);
            await sleep(140);
            panel2 = findSettingsPanel();
            if (!panel2) return true;
        ...
        const pickSelectValue = async (labelText, valueText, fallbackIndex) => {
          ...
          for (let attempt = 0; attempt < 4; attempt += 1) {
            ...
            let field = fields.find((el2) => {
              const txt = norm(el2.getAttribute("aria-label") || el2.textContent);
...
async function flowSimulateImageToVideo(tabId, imagePrompt, videoPrompt, projectId, createImageOptions) {
  ...
  const stepWaitImage = await flowWaitForImageReady(tabId, projectId, baselineTiles.count, 9e4);
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (actionName, actionValue) => {
      ...
      const openMainMenu = async () => {
        ...
        const expanded = String(controlBtn.getAttribute("aria-expanded") || "").toLowerCase() === "true" || String(controlBtn.getAttribute("data-state") || "").toLowerCase() === "open";
        if (!expanded) {
        ...
        for (let i = 0; i < 10; i += 1) {
          ...
          const menu2 = (byId && isVisible(byId) ? byId : null) || openMenus.find((m) => m.querySelector(".flow_tab_slider_trigger")) || openMenus[0] || null;
          if (menu2) {
            return { ok: true, menu: menu2, controlBtn, controlsId };
          }
          await sleep(120);
        }
      ...
      const clickTabInScope = async (scope, wantedLabel) => {
        ...
        await sleep(120);
        return String(target.getAttribute("aria-selected") || "").toLowerCase() === "true" || String(target.getAttribute("data-state") || "").toLowerCase() === "active";
      };
      const findTablistByTokens = (menu2, tokens) => {
        const tablists = Array.from(menu2.querySelectorAll('[role="tablist"]')).filter(isVisible);
      ...
      const clickTabByTablistTokens = async (menu2, tablistTokens, wantedLabel) => {
        const tablist = findTablistByTokens(menu2, tablistTokens);
      ...
      const setModelFromMenu = async (menu2, wantedModel) => {
        const modelBtn = Array.from(menu2.querySelectorAll('button[aria-haspopup="menu"]')).filter(isVisible)[0] || null;
        ...
        await sleep(150);
        const parts = parseModelParts(wantedModel).map((x2) => compact(x2)).filter(Boolean);
        ...
        for (let i = 0; i < 20; i += 1) {
          ...
          const candidateMenus = byId && isVisible(byId) ? [byId] : openMenus.filter((m) => m !== menu2);
          let submenu = null;
          let option = null;
          let optionLabels = [];
      ...
      if (actionName === "select_video_submode") {
        ...
        return ok ? { success: true, method: "controls_menu", submode: normalized } : { success: false, error: `Video submode '${normalized}' not found in controls menu` };
      }
      ...
      if (actionName === "set_model") {
        ...
        return picked.ok ? { success: true, method: "controls_menu" } : { success: false, error: picked.error || "Failed to set model", labels: picked.labels || [] };
      }
...
async function flowSelectImageTab(tabId, projectId) {
  ...
  const verifyImageContext = async () => {
    const probeResults2 = await chrome.scripting.executeScript({
      target: { tabId: resolvedTabId },
      func: () => {
        const norm = (txt) => String(txt || "").toLowerCase().replace(/\s+/g, " ").trim();
        const isVisible = (el2) => {
        ...
        const labels = Array.from(document.querySelectorAll('button, [role="button"], [role="tab"], [aria-label]')).filter(isVisible).map((el2) => norm(el2.textContent || el2.getAttribute("aria-label") || ""));
        const pageText = norm(document.body?.textContent || "");
        const inImageUi = labels.some((txt) => txt === "ref" || txt.includes("add reference") || txt.includes("create image") || txt.includes("image generation")) || pageText.includes("create image") || pageText.includes("edit image") || pageText.includes("image generation");
        return {
          success: true,
          inImageUi,
    ...
    const probe2 = normalizeFlowScriptResult(probeResults2, resolvedTabId, "flow_select_image_tab_verify");
    return Boolean(probe2.success === true && probe2.inImageUi === true);
  };
  for (let attempt = 0; attempt < 3; attempt += 1) {
  ...
  const probeResults = await chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const isImageContext = () => {
        ...
        return hints.some((el2) => {
          const txt = norm(el2.textContent || el2.getAttribute("aria-label") || "");
          if (!txt) return false;
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const findIngredientsButton = () => {
        ...
        return buttons.find((el2) => {
          const text = norm(el2.textContent || el2.getAttribute("aria-label") || "");
...
async function flowSelectReferenceMode(tabId, projectId) {
  ...
  const ensureReferenceContext = async () => {
    const checkResults = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        const hasReferenceControls = Array.from(document.querySelectorAll('button, [role="button"], [role="tab"], [aria-label]')).filter(isVisible).some((el2) => {
          const txt = norm(el2.textContent || el2.getAttribute("aria-label") || "");
  ...
  const runOnce = async () => chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const isReferenceButton = (el2) => {
        if (!el2 || !isVisible(el2)) return false;
        const text2 = norm(el2.textContent || el2.getAttribute("aria-label") || "");
        if (!text2) return false;
        if (text2.includes("preference") || text2.includes("reference docs")) return false;
        return text2 === "ref" || text2.includes(" ref ") || text2.includes("reference") || text2.includes("add reference");
      };
      const findReferenceButton = () => {
        const composerRoots = Array.from(document.querySelectorAll("form, section, article, div")).filter((el2) => {
          const txt = norm(el2.textContent || "");
      ...
      const findAddMediaTrigger = () => {
        ...
        return buttons.find((el2) => {
          const ariaHasPopup = String(el2.getAttribute("aria-haspopup") || "").toLowerCase();
          if (ariaHasPopup !== "dialog") return false;
          const iconText = norm(Array.from(el2.querySelectorAll("i, span, img[alt]")).map((n) => n.textContent || n.getAttribute?.("alt") || "").join(" "));
          const txt = norm(el2.textContent || el2.getAttribute("aria-label") || "");
      ...
      const getOpenAssetRows = (controlsId) => {
        ...
        const list = scoped || document.querySelector('[role="dialog"][data-state="open"] [data-testid="virtuoso-item-list"]') || document.querySelector('[role="dialog"] [data-testid="virtuoso-item-list"]') || document.querySelector('[data-testid="virtuoso-item-list"]');
        if (!list || !isVisible(list)) return [];
  ...
  const runSlotPick = async () => {
    const results = await chrome.scripting.executeScript({
      ...
      func: async (wantedSlot) => {
        ...
        const readSlotState = (el2) => {
          const selectedHint = String(el2?.getAttribute("aria-pressed") || el2?.getAttribute("aria-selected") || "").toLowerCase();
          const dataState = String(el2?.getAttribute("data-state") || "").toLowerCase();
          const expanded = String(el2?.getAttribute("aria-expanded") || "").toLowerCase();
          return {
        ...
        const findFrameSlotButton = () => {
          const isSlotControlCandidate = (el2) => {
            if (!el2 || !isVisible(el2)) return false;
            if (el2.hasAttribute("data-card-open") || el2.closest("[data-card-open]")) return false;
            const label = norm(el2.getAttribute("aria-label") || "");
            const text = norm(el2.textContent || "");
          ...
          const exactDialogTrigger = dialogTriggers.find((btn2) => {
            const text = norm(btn2.textContent || btn2.getAttribute("aria-label") || "");
          ...
          const exact = buttons.find((btn2) => {
            const text = norm(btn2.textContent || btn2.getAttribute("aria-label") || "");
            return text === wantedSlot;
          });
          if (exact) return exact;
          return buttons.find((btn2) => {
            const text = norm(btn2.textContent || btn2.getAttribute("aria-label") || "");
        ...
        if (!btn) {
          const available = Array.from(document.querySelectorAll('[aria-haspopup="dialog"], div[type="button"], button, [role="button"]')).filter(isVisible).map((el2) => norm(el2.textContent || el2.getAttribute("aria-label") || "")).filter(Boolean).slice(0, 20);
  ...
  try {
    ...
    const extracted = await chrome.scripting.executeScript({
      ...
      func: async (projectId2, assetId2) => {
        const fetchWithTimeout = async (url, options = {}, timeoutMs = 8e3) => {
        ...
        const imgSources = Array.from(document.querySelectorAll("img")).slice(0, 80).map((img) => String(img.getAttribute("src") || img.getAttribute("data-src") || img.currentSrc || "").trim());
        ...
        const scriptPayloads = Array.from(document.querySelectorAll('script[type="application/ld+json"], script[type="application/json"]')).slice(0, 40).map((script) => String(script.textContent || "").trim()).filter(Boolean);
        ...
        try {
          const response = await fetchWithTimeout(trpcPath, { credentials: "include", redirect: "follow" }, 8e3);
        ...
        try {
          const editUrl2 = `https://labs.google/fx/tools/flow/project/${encodeURIComponent(String(projectId2 || ""))}/edit/${encodeURIComponent(String(assetId2 || ""))}`;
          const response = await fetchWithTimeout(editUrl2, { credentials: "include", redirect: "follow" }, 8e3);
          strings.push(String(response.url || ""));
          const bodyText = await response.text();
          if (bodyText) strings.push(String(bodyText));
        } catch {
...
async function flowSelectAsset(tabId, assetId, projectId, slot, assetIndex, assetExactId) {
  ...
  const aliasIdsFromEditPage = rawAssetExactId ? await resolveFlowAssetAliasesFromEditPage(normalizedProjectId, rawAssetExactId) : [];
  if (!targetAssetId) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (wantedAssetId, wantedCompactId, wantedSlot, wantedAssetIndex, wantedAssetExactId, wantedProjectId, wantedAliasIds) => {
      ...
      const parseMediaNameFromSrc = (src) => {
        ...
        const match2 = raw.match(/[?&]name=([^&#]+)/i);
        if (!match2?.[1]) return "";
        try {
          return decodeURIComponent(match2[1]);
        } catch {
          return String(match2[1]);
        }
      };
      ...
      const wantedAliasSet = new Set(
        Array.isArray(wantedAliasIds) ? wantedAliasIds.map((item) => String(item || "").trim().toLowerCase()).filter(Boolean) : []
      );
      const wantedIndex = typeof wantedAssetIndex === "number" && Number.isInteger(wantedAssetIndex) ? wantedAssetIndex : null;
      const matchMode = "strict_exact";
      const findFramesBar = () => {
        const containers = Array.from(document.querySelectorAll("div, section, article")).filter(isVisible);
        const bar = containers.find((el2) => {
          const text = norm(el2.textContent || "");
          if (text.includes("swap first and last frames")) return true;
          const icon = norm(Array.from(el2.querySelectorAll("i, span")).map((n) => n.textContent || "").join(" "));
      ...
      const clickButtonLikeUser = (el2) => {
        if (!el2) return;
        const overlay = el2.querySelector('[data-type="button-overlay"]');
        const target2 = overlay || el2;
        target2.dispatchEvent(new MouseEvent("pointerdown", { bubbles: true }));
        target2.dispatchEvent(new MouseEvent("mousedown", { bubbles: true }));
        target2.dispatchEvent(new MouseEvent("mouseup", { bubbles: true }));
      ...
      const isSlotDialogTrigger = (el2, slotName) => {
        if (!slotName || !el2 || !isVisible(el2)) return false;
        if (el2.hasAttribute("data-card-open") || el2.closest("[data-card-open]")) return false;
        if (String(el2.getAttribute("aria-haspopup") || "").toLowerCase() !== "dialog") return false;
        const text = norm(el2.textContent || el2.getAttribute("aria-label") || "");
      ...
      const findSlotDialogTrigger = (slotName) => {
        ...
        if (bar) {
          ...
          const exactInBar = inBar.find((el2) => isSlotDialogTrigger(el2, slotName));
        ...
        const exactLocal = local.find((el2) => isSlotDialogTrigger(el2, slotName));
        if (exactLocal) return exactLocal;
        const global2 = Array.from(document.querySelectorAll('[aria-haspopup="dialog"], div[type="button"][aria-haspopup="dialog"], button[aria-haspopup="dialog"], [role="button"][aria-haspopup="dialog"]'));
        return global2.find((el2) => isSlotDialogTrigger(el2, slotName)) || null;
      };
      ...
      const getOpenAssetRows = (controlsId2) => {
        const byId = controlsId2 ? document.getElementById(controlsId2) : null;
        const scopedList = byId?.querySelector('[data-testid="virtuoso-item-list"]');
        const list = scopedList || document.querySelector('[role="dialog"][data-state="open"] [data-testid="virtuoso-item-list"]') || document.querySelector('[role="dialog"] [data-testid="virtuoso-item-list"]') || document.querySelector('[data-testid="virtuoso-item-list"]');
        if (!list || !isVisible(list)) return [];
        const rows2 = Array.from(list.querySelectorAll('[data-index], [data-asset-id], [data-index="0"]')).filter(isVisible);
        return rows2.length ? rows2 : Array.from(list.querySelectorAll("div, li, button")).filter(isVisible);
      };
      const getAssetList = async (controlsId2) => {
        for (let attempt = 0; attempt < 6; attempt += 1) {
          const rows2 = getOpenAssetRows(controlsId2);
          if (rows2.length > 0) return rows2;
          await sleep(140);
        }
        return [];
      };
      const ensureAssetRowsReady = async (trigger2, controlsId2) => {
        let rows2 = await getAssetList(controlsId2);
        if (rows2.length > 0) return rows2;
        for (let attempt = 0; attempt < 12; attempt += 1) {
          if (trigger2 && (attempt === 0 || attempt % 3 === 2)) {
            clickButtonLikeUser(trigger2);
            await sleep(180);
          }
          rows2 = getOpenAssetRows(controlsId2);
          if (rows2.length > 0) return rows2;
          await sleep(180);
        }
        return rows2;
      };
      const closeAssetDialog = async (trigger2, controlsId2) => {
        const dialog = controlsId2 ? document.getElementById(controlsId2) : null;
        if (dialog && isVisible(dialog) && trigger2) {
          clickButtonLikeUser(trigger2);
          await sleep(140);
          return true;
        }
        const openDialog = document.querySelector('[role="dialog"][data-state="open"]');
        if (openDialog && isVisible(openDialog) && trigger2) {
          clickButtonLikeUser(trigger2);
          await sleep(140);
          return true;
        }
      ...
      const findAssetLibraryTrigger = (slotName) => {
        ...
        const localOpen = local.find((el2) => {
          const expanded = norm(el2.getAttribute("aria-expanded") || "");
          const state = norm(el2.getAttribute("data-state") || "");
          return expanded === "true" || state === "open";
        });
        if (localOpen) return localOpen;
        const localMatch = local.find((el2) => {
          const controlsId2 = norm(el2.getAttribute("aria-controls") || "");
          const iconText = norm(Array.from(el2.querySelectorAll("i, span")).map((n) => n.textContent || "").join(" "));
          const label = norm(el2.getAttribute("aria-label") || el2.textContent || "");
          return controlsId2.startsWith("radix-") && (iconText.includes("add_2") || label === "create" || label.includes("add") || label === "start" || label === "end");
        });
        if (localMatch) return localMatch;
        const global2 = Array.from(document.querySelectorAll('button[aria-haspopup="dialog"], div[aria-haspopup="dialog"], [role="button"][aria-haspopup="dialog"], div[type="button"][aria-haspopup="dialog"]')).filter(isVisible);
        const globalOpen = global2.find((el2) => {
          const expanded = norm(el2.getAttribute("aria-expanded") || "");
          const state = norm(el2.getAttribute("data-state") || "");
          return expanded === "true" || state === "open";
        });
        if (globalOpen) return globalOpen;
        const globalMatch = global2.find((el2) => {
          const controlsId2 = norm(el2.getAttribute("aria-controls") || "");
          const iconText = norm(Array.from(el2.querySelectorAll("i, span")).map((n) => n.textContent || "").join(" "));
          const label = norm(el2.getAttribute("aria-label") || el2.textContent || "");
          return controlsId2.startsWith("radix-") && (iconText.includes("add_2") || label === "create" || label.includes("add") || label === "start" || label === "end");
      ...
      const toCandidate = (row) => {
        ...
        const altText = String(Array.from(row.querySelectorAll("img[alt]")).map((img2) => img2.getAttribute("alt") || "").join(" ")).replace(/\s+/g, " ").trim();
        ...
        const tileId = String(
          row.getAttribute("data-tile-id") || row.closest("[data-tile-id]")?.getAttribute("data-tile-id") || ""
        ).trim();
      ...
      const getDialogRoot = (controlsId2) => {
        const byId = controlsId2 ? document.getElementById(controlsId2) : null;
      ...
      const getAssetPanelRoots = (controlsId2) => {
        const roots = [];
        const dialogRoot = getDialogRoot(controlsId2);
      ...
      const inferProjectIdFromLocation = () => {
        ...
        const match2 = text.match(/\/project\/([0-9a-f-]{36})/i);
        return String(match2?.[1] || "").trim();
      };
      const extractUuidSet = (text) => {
        const ids = /* @__PURE__ */ new Set();
        if (!text) return ids;
        const matches2 = String(text).match(uuidRegex) || [];
        for (const match2 of matches2) ids.add(String(match2).toLowerCase());
        return ids;
      };
      ...
      const collectImageCandidates = (controlsId2) => {
        const roots = getAssetPanelRoots(controlsId2);
        if (!roots.length) return [];
        const imgs = roots.flatMap((root) => Array.from(root.querySelectorAll("img[alt]")).filter(isVisible));
        const raw = imgs.map((img) => {
      ...
      const collectVisibleListAliasEntries = () => {
        ...
        for (const root of roots) {
          ...
          const mapped = anchors.map((anchor) => {
            ...
            const src = String(
              nearestImg?.getAttribute("src") || nearestImg?.getAttribute("data-src") || nearestImg?.currentSrc || ""
            ).trim();
            const nameId = parseMediaNameFromSrc(src);
            const srcUuids = Array.from(extractUuidSet(src));
            const tileId = String(
              anchor.closest("[data-tile-id]")?.getAttribute("data-tile-id") || nearestImg?.closest("[data-tile-id]")?.getAttribute("data-tile-id") || rootTileId || ""
            ).trim();
            const feUuid = parseFeUuidFromTileId(tileId);
            return {
              editId,
      ...
      const visibleAliasSet = new Set(
        visibleAliasEntries.filter((entry) => targetUuidForAlias && (String(entry.editId || "").trim().toLowerCase() === targetUuidForAlias || String(entry.nameId || "").trim().toLowerCase() === targetUuidForAlias || String(entry.feUuid || "").trim().toLowerCase() === targetUuidForAlias)).flatMap((entry) => [
      ...
      const isExactMatch = (meta) => {
        ...
        return meta.labelNormalized === wantedNormalized && Boolean(wantedNormalized) || meta.labelCompact === wantedCompact && Boolean(wantedCompact) || meta.id === wantedAssetId || meta.idNormalized === wantedNormalized || meta.idCompact && meta.idCompact === wantedCompact && Boolean(wantedCompact) || meta.editId && meta.editId.toLowerCase() === wantedAssetId.toLowerCase() || meta.editIdNormalized === wantedNormalized || meta.editIdCompact && meta.editIdCompact === wantedCompact && Boolean(wantedCompact) || meta.feUuid && meta.feUuid.toLowerCase() === wantedAssetId.toLowerCase() || meta.feUuidNormalized === wantedNormalized || meta.feUuidCompact && meta.feUuidCompact === wantedCompact && Boolean(wantedCompact) || Array.isArray(meta.editUuids) && meta.editUuids.some((id) => String(id || "").trim().toLowerCase() === wantedAssetId.toLowerCase()) || meta.dataIndex === wantedAssetId || meta.dataIndexNormalized === wantedNormalized || meta.dataIndexCompact && meta.dataIndexCompact === wantedCompact && Boolean(wantedCompact) || meta.altText && meta.altText.toLowerCase() === wantedAssetId.toLowerCase() || meta.altNormalized === wantedNormalized || meta.altCompact && meta.altCompact === wantedCompact;
      };
      ...
      const uniqueContainsMatches = candidates.filter((meta) => {
        ...
        return meta.labelCompact && meta.labelCompact.includes(wantedCompact) || meta.altCompact && meta.altCompact.includes(wantedCompact) || meta.idCompact && meta.idCompact.includes(wantedCompact) || meta.dataIndexCompact && meta.dataIndexCompact === wantedCompact;
      });
      const resolvedMatches = matches.length > 0 ? matches : uniqueContainsMatches.length === 1 ? uniqueContainsMatches : [];
      const exactIdMatches = wantedExactId ? candidates.filter((meta) => {
      ...
      const aliasIdMatches = mergedAliasSet.size > 0 ? candidates.filter((meta) => {
        if (mergedAliasSet.has(String(meta.id || "").trim().toLowerCase())) return true;
        if (mergedAliasSet.has(String(meta.editId || "").trim().toLowerCase())) return true;
        if (Array.isArray(meta.editUuids) && meta.editUuids.some((id) => mergedAliasSet.has(String(id || "").trim().toLowerCase()))) return true;
        return Array.isArray(meta.srcUuids) && meta.srcUuids.some((id) => mergedAliasSet.has(String(id || "").trim().toLowerCase()));
      }) : [];
      const indexMatches = wantedIndex !== null ? resolvedMatches.filter((meta) => Number.parseInt(String(meta.dataIndex || ""), 10) === wantedIndex) : [];
      const resolvedMatchMode = matches.length > 0 ? matchMode : uniqueContainsMatches.length === 1 ? "strict_contains_unique" : matchMode;
      ...
      const canonicalCandidateLabel = (meta) => {
        ...
        const isGeneric = (value) => {
          ...
          if (!text) return true;
          return text.includes("a piece of media generated or uploaded by you") || text === "image" || text === "video";
        };
        const isNoisy = (value) => {
      ...
      const selectCandidateAndVerify = async (match2, mode, extra = {}) => {
        const target2 = match2.row.firstElementChild || match2.row;
        clickButtonLikeUser(target2);
        await sleep(260);
        const dialogClosed2 = await closeAssetDialog(trigger, controlsId);
        let afterSlotAttached2 = isSlotAttached(wantedSlot);
        if (wantedSlot) {
          for (let attempt = 0; attempt < 10 && afterSlotAttached2 !== true; attempt += 1) {
            await sleep(120);
            afterSlotAttached2 = isSlotAttached(wantedSlot);
          }
          ...
          if (expectedExactId && verifiedAttachedAssetId && verifiedAttachedAssetId !== expectedExactId) {
            return {
              ...
              requestedAssetIndex: wantedIndex,
              selectedAssetId: preferredSelectedAssetId(match2),
              slot: wantedSlot,
              slotAttachedBefore: beforeSlotAttached,
              slotAttachedAfter: afterSlotAttached2,
              slotAttachedAssetIdAfter: verifiedAttachedAssetId || null,
              candidateCount: candidates.length,
              matchedCount: resolvedMatches.length,
              dialogClosed: dialogClosed2,
              dialogControlsId: controlsId || null,
              ...extra
            };
          }
          if (afterSlotAttached2 !== true) {
            return {
              success: false,
        ...
        return {
          ...
          requestedAssetIndex: wantedIndex,
          selectedAssetId: preferredSelectedAssetId(match2),
          selectedIndex: match2.dataIndex || null,
          selectedText: canonicalCandidateLabel(match2),
        ...
        if (exactIdMatches.length === 0) {
          ...
          if (combinedAliasSet.size > 0) {
            ...
            if (combinedAliasMatches.length === 1) {
              ...
              const target2 = aliasMatch.row.firstElementChild || aliasMatch.row;
              clickButtonLikeUser(target2);
              await sleep(260);
              const dialogClosed2 = await closeAssetDialog(trigger, controlsId);
              let afterSlotAttached2 = isSlotAttached(wantedSlot);
          ...
          const available = candidates.slice(0, 24).map((c) => ({
      ...
      if (resolvedMatches.length > 1) {
        ...
        if (deterministic) {
          const match2 = deterministic.match;
          const target2 = match2.row.firstElementChild || match2.row;
          clickButtonLikeUser(target2);
          await sleep(260);
          const dialogClosed2 = await closeAssetDialog(trigger, controlsId);
  ...
  const selectedMatchesRequestedExact = requestedExactId ? String(selectedCardIdentity.assetId || "").toLowerCase() === requestedExactId.toLowerCase() : null;
...
async function flowTypePrompt(tabId, text, projectId, clearBeforeType = false) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (message, clearBeforeTyping) => {
      ...
      let input = composerContainer ? Array.from(composerContainer.querySelectorAll('div[role="textbox"][contenteditable="true"][data-slate-editor="true"], [role="textbox"][contenteditable="true"], textarea')).find((el2) => isVisible(el2) && !isEditImageContext(el2)) || null : null;
      if (!input) {
        const fallbackInputs = Array.from(document.querySelectorAll('div[role="textbox"][contenteditable="true"][data-slate-editor="true"], [role="textbox"][contenteditable="true"][aria-multiline="true"], textarea')).filter((el2) => isVisible(el2) && !isEditImageContext(el2));
        input = fallbackInputs[0] || null;
      }
      if (!input) {
        return {
...
async function flowGenerateVideo(tabId, projectId) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const isGenerateVideoButton = (el2) => {
        if (!el2 || !isVisible(el2)) return false;
        if (el2.closest('[role="menu"][data-radix-menu-content]')) return false;
        const text = norm(el2.textContent || el2.getAttribute("aria-label") || "");
        const iconText = norm(el2.querySelector(".material-symbols-outlined, .material-icons")?.textContent || "");
        const hasMenu = String(el2.getAttribute("aria-haspopup") || "").toLowerCase();
      ...
      const detectGenerationSignal = () => {
        ...
        const percentNode = Array.from(document.querySelectorAll('span, div, p, [role="status"]')).find((el2) => isVisible(el2) && /\b\d{1,3}%\b/.test(String(el2.textContent || "").trim()));
...
async function flowGenerateImage(tabId, projectId) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const composerRoot = composerRoots.find(isVisible) || composerRoots[0] || document.body;
      const isGenerateButton = (el2) => {
        if (!el2 || !isVisible(el2)) return false;
        const text = norm(el2.textContent || el2.getAttribute("aria-label") || "");
        const iconText = norm(el2.querySelector(".material-symbols-outlined, .material-icons")?.textContent || "");
        const add2DialogTrigger = String(el2.getAttribute("aria-haspopup") || "").toLowerCase() === "dialog" && norm(el2.querySelector('img[alt="add_2"]')?.getAttribute("alt") || "") === "add_2";
...
async function getTabDom(tabId, options = {}) {
  ...
  const maxTextLength = Number.isFinite(maxTextLengthRaw) ? Math.min(Math.max(Math.trunc(maxTextLengthRaw), 20), 2e3) : 240;
  const maxHtmlLengthRaw = Number(options.maxHtmlLength);
  const maxHtmlLength = Number.isFinite(maxHtmlLengthRaw) ? Math.min(Math.max(Math.trunc(maxHtmlLengthRaw), 50), 8e3) : 600;
  const results = await chrome.scripting.executeScript({
    target: { tabId: resolvedTabId },
    func: (opts) => {
      const isVisible = (el2) => {
      ...
      const toNode = (el2, idx) => {
        const rect = el2.getBoundingClientRect();
        const style = window.getComputedStyle(el2);
        const text = cleanText(el2.innerText || el2.textContent || "", opts.maxTextLength);
        const node = {
          index: idx,
          tag: String(el2.tagName || "").toLowerCase(),
          id: el2.id || null,
          className: el2.className || null,
          role: el2.getAttribute("role") || null,
          ariaLabel: el2.getAttribute("aria-label") || null,
          name: el2.getAttribute("name") || null,
          type: el2.getAttribute("type") || null,
          placeholder: el2.getAttribute("placeholder") || null,
          href: el2.getAttribute("href") || null,
          text,
          visible: isVisible(el2),
          rect: {
            x: Math.round(rect.x),
            y: Math.round(rect.y),
        ...
        if (opts.includeHtml) {
          node.outerHTML = String(el2.outerHTML || "").slice(0, opts.maxHtmlLength);
        }
      ...
      const filtered = opts.includeHidden ? all : all.filter((el2) => isVisible(el2));
      const limited = filtered.slice(0, opts.maxNodes);
      const nodes = limited.map((el2, idx) => toNode(el2, idx));
      return {
        success: true,
...
async function getTabHtml(tabId, maxChars) {
  ...
  const resolvedMaxChars = Number.isFinite(maxCharsRaw) ? Math.min(Math.max(Math.trunc(maxCharsRaw), 1e3), 5e5) : 12e4;
  const results = await chrome.scripting.executeScript({
    target: { tabId: resolvedTabId },
    func: (limit) => {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (message) => {
      ...
      if (stillHasMessage) {
        ...
        const stillAfterRetry = isContentEditable ? String(input.textContent || "").trim().length > 0 : String(input.value || "").trim().length > 0;
        if (stillAfterRetry) {
...
async function injectStatusBadge(tabId) {
  ...
  await chrome.scripting.executeScript({
    ...
    func: (status, tid) => {
      ...
      const bg = online ? "linear-gradient(135deg, rgba(7,105,84,0.94), rgba(16,185,129,0.94))" : "linear-gradient(135deg, rgba(127,29,29,0.94), rgba(239,68,68,0.94))";
      const border = online ? "1px solid rgba(110,231,183,0.75)" : "1px solid rgba(254,202,202,0.72)";
      const badge = document.createElement("div");
...
async function injectStatusBadgeToAllGeminiTabs() {
  const tabs = await chrome.tabs.query({ url: "https://gemini.google.com/*" });
  await Promise.all(
    tabs.filter((tab) => Number.isInteger(tab.id)).map((tab) => injectStatusBadge(tab.id).catch(() => null))
  );
}
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (targetModelRaw) => {
      ...
      for (let i = 0; i < 8; i += 1) {
        ...
        const activeMenu = menuRoots.find((m) => m.querySelector('[role="menuitemradio"]') && m.offsetParent !== null) || menuRoots.reverse().find((m) => m.querySelector('[role="menuitemradio"]')) || null;
        const radioOptions = activeMenu ? Array.from(activeMenu.querySelectorAll('[role="menuitemradio"]')).filter(isInteractable) : Array.from(document.querySelectorAll('[role="menuitemradio"]')).filter(isInteractable);
      ...
      if (!match) {
        ...
        if (targetModel === "pro") {
          const pageText2 = normalizeLabel(document.body?.innerText || "");
          const upsellDetected2 = /upgrade to google ai plus/.test(pageText2);
          if (upsellDetected2) {
            return {
              success: false,
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async () => {
      ...
      const ensureToolsButton = async () => {
        ...
        if (!found) {
          const newChat = Array.from(document.querySelectorAll("a,button")).find((el2) => {
            const label = normalize(el2.getAttribute("aria-label") || el2.textContent);
      ...
      if (!toolsButton) {
        return {
          ...
          buttons: ensured.visibleButtons.map((b) => normalize(b.getAttribute("aria-label") || b.textContent)).filter(Boolean).slice(0, 20)
        };
      }
      toolsButton.click();
      await sleep(180);
      ...
      const items = rawItems.map((el2) => {
        const label = normalize(el2.textContent);
        const disabled = el2.getAttribute("aria-disabled") === "true" || el2.hasAttribute("disabled");
        return {
          label,
          disabled,
          role: el2.getAttribute("role") || "menuitem"
        };
      }).filter((x2) => x2.label.length > 0);
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: async (target) => {
      ...
      const ensureToolsButton = async () => {
        let visibleButtons = Array.from(document.querySelectorAll("button")).filter(isVisible);
        let found = visibleButtons.find((btn) => {
          const label2 = normalize(btn.getAttribute("aria-label") || btn.textContent);
          return /\btools\b/i.test(label2);
        });
        if (!found) {
          const newChat = Array.from(document.querySelectorAll("a,button")).find((el2) => {
            const label2 = normalize(el2.getAttribute("aria-label") || el2.textContent);
            return /\bnew\s+chat\b/i.test(label2);
          });
          if (newChat) {
            newChat.click();
            await sleep(400);
...
async function createWithTool(tabId, tool, prompt, accountIndex) {
  ...
  if (!finalToolResult.success && String(finalToolResult.error || "").includes("Tools button not found")) {
    const freshTab = await createGeminiTab(geminiAppUrl(accountIndex), void 0, accountIndex);
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse2) => {
  if (msg?.action === "reconnect") {
    connectMQTT();
    sendResponse2({ success: true, connected });
    return true;
  }
  ...
  if (msg?.action === "list_gemini_tabs") {
    listGeminiTabs().then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "list_flow_tabs") {
    listFlowTabs().then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "list_tabs_all") {
    listAllTabs().then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "open_gemini_tab") {
    createGeminiTab(msg?.url, msg?.mode, parseAccountIndex(msg?.accountIndex)).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "open_flow_tab") {
    createFlowTab(msg?.url, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "open_flow_project") {
    openFlowProject(msg?.projectId, msg?.tabId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_current_project") {
    flowCurrentProject(msg?.tabId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_new_project") {
    flowNewProject(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_tool") {
    flowSelectTool(msg?.tabId, msg?.tool, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_submit_prompt") {
    flowSubmitPrompt(msg?.tabId, msg?.prompt || msg?.text, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_configure_create_image") {
    flowConfigureCreateImage(
      ...
      msg?.projectId
    ).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_simulate_image_to_video") {
    flowSimulateImageToVideo(msg?.tabId, msg?.imagePrompt, msg?.videoPrompt, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_video_tab") {
    flowSelectVideoTab(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_image_tab") {
    flowSelectImageTab(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_ingredients_mode") {
    flowSelectIngredientsMode(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_reference_mode") {
    flowSelectReferenceMode(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_frames_mode") {
    flowSelectFramesMode(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_frame_slot") {
    flowSelectFrameSlot(msg?.tabId, msg?.slot, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_asset") {
    flowSelectAsset(msg?.tabId, msg?.assetId, msg?.projectId, msg?.slot, msg?.assetIndex, msg?.assetExactId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_select_latest_image_ingredient") {
    flowSelectLatestImageIngredient(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_set_model") {
    flowSetModel(msg?.tabId, msg?.model, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_set_multiplier") {
    flowSetMultiplier(msg?.tabId, msg?.multiplier, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_set_aspect_ratio") {
    flowSetAspectRatio(msg?.tabId, msg?.ratio, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_type_prompt") {
    flowTypePrompt(msg?.tabId, msg?.text, msg?.projectId, msg?.clearBeforeType).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_generate_video") {
    flowGenerateVideo(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "flow_generate_image") {
    flowGenerateImage(msg?.tabId, msg?.projectId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "get_url") {
    getTabUrl(msg?.tabId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "get_text") {
    getTabText(msg?.tabId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  ...
  if (msg?.action === "get_html") {
    getTabHtml(msg?.tabId, msg?.maxChars).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "get_state") {
    getTabState(msg?.tabId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "inject_status_badge") {
    injectStatusBadge(msg?.tabId).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  if (msg?.action === "select_model") {
    selectModel(msg?.tabId, msg?.model, parseAccountIndex(msg?.accountIndex)).then((result) => sendResponse2(result)).catch((err) => sendResponse2({ success: false, error: err instanceof Error ? err.message : String(err) }));
    return true;
  }
  return false;
});
