...
import {
  CUSTOM_GEM_ACTION,
  CUSTOM_GEM_BASE_SEND_STABLE_MS,
  ...
  CUSTOM_GEM_SEND_READY_GRACE_MS,
...
const CUSTOM_GEM_REPLAY_LIMIT = 64;
...
const customGemFollowupInFlight = new Map();
const customGemTabLocks = new Map();
const customGemQuarantinedTabs = new Map();
let customGemDurableQueue = Promise.resolve();
...
function sendResponse(id, data) {
  ...
  const isExactRecoverySuccess = data?.version === CUSTOM_GEM_RECOVERY_RESULT_VERSION &&
    data?.state === 'succeeded';
  const isExactRevisionSuccess = data?.version === CUSTOM_GEM_REVISION_RESULT_VERSION &&
    data?.state === 'succeeded';
...
async function handleCommand(cmd) {
  ...
  try {
    ...
    if (cmd.action === CUSTOM_GEM_RECOVER_ACTION) {
      ...
      result = cmd.action === CUSTOM_GEM_ACTION || cmd.action === CUSTOM_GEM_SUBMIT_ACTION
        ? customGemFailure(
          typeof cmd.requestId === 'string' ? cmd.requestId : null,
          'handler',
          'handler_error',
...
function customGemPageDetails(payload) {
  ...
  if (Array.isArray(payload.controlLabels)) {
    const controlLabels = payload.controlLabels
...
function withCustomGemDurableLock(operation) {
  ...
  customGemDurableQueue = task.catch(() => undefined);
  return task;
}
...
function compactCustomGemDurableRequests(current) {
  const completedWithResults = Object.entries(current)
    .filter(([, value]) => value?.status === 'completed' && value.result)
...
function verifyTrustedCustomGemUploadReceipt(request) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(customGemBrowserError('upload_attestation_rejected')), 5000);
    chrome.runtime.sendMessage(request.uploadReceipt.browserControlExtensionId, payload, (response) => {
      ...
      if (chrome.runtime.lastError || response?.ok !== true || response?.result?.consumed !== true ||
          ...
          response.result.browserControlExtensionId !== request.uploadReceipt.browserControlExtensionId ||
...
function verifyTrustedCustomGemFollowupReceipt(request) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(customGemBrowserError('followup_attestation_rejected')), 5000);
    chrome.runtime.sendMessage(request.authorizationReceipt.browserControlExtensionId, payload, (response) => {
      clearTimeout(timer);
      if (chrome.runtime.lastError || response?.ok !== true || response?.result?.consumed !== true ||
          response.result.receiptId !== request.authorizationReceipt.receiptId ||
          response.result.browserControlExtensionId !== request.authorizationReceipt.browserControlExtensionId ||
          response.result.gemId !== request.gemId ||
          response.result.gemUrl !== request.gemUrl ||
          response.result.currentUrl !== request.currentUrl ||
          response.result.conversationUrl !== request.conversationUrl) {
...
function customGemPreflightPage(expectedUrl) {
  ...
  const findVisible = (root, selectors, predicate) => {
    const result = [];
  ...
  const responseSelector = responseSelectors.find((selector) =>
...
async function customGemAttachPage(payload) {
  ...
  if (existingInputs.some((fileInput) => fileInput.files?.length > 0)) {
  ...
  if (!fileInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 50; attempt += 1) {
        ...
        const menuCandidates = unique(Array.from(document.querySelectorAll('button[role^="menuitem"], button[role="option"], button[aria-label], [role="button"][aria-label]')))
          .filter(isVisible)
          .filter((candidate) => candidate !== uploadCandidates[0])
          .filter((candidate) => /upload|attach|files?|computer|device/i.test(labelOf(candidate)) && !/download/i.test(labelOf(candidate)));
...
async function awaitTrustedCustomGemResponsePage(payload) {
  ...
  while (Date.now() < deadlineAt) {
    ...
    const matchingMessages = currentMessages.filter(
      (node) =>
        node.getAttribute('data-custom-gem-baseline') !== attributionToken &&
        normalizeText(messageText(node)) === normalizeText(message),
    );
    ...
    const nextMessageNode = currentMessages.find(
      (node) => node !== attributedMessageNode &&
        Boolean(attributedMessageNode.compareDocumentPosition(node) & Node.DOCUMENT_POSITION_FOLLOWING),
    ) || null;
...
async function customGemFollowupPreparePage(payload) {
  ...
  const message = String(payload?.message || '');
  const deadlineAt = Number(payload?.deadlineAt || 0);
  const sendStableMs = Number(payload?.sendStableMs || 2000);
  const sendReadyWindowMs = Number(payload?.sendReadyWindowMs || 12000);
...
async function executeCustomGemRecover(cmd) {
  ...
  try {
    ...
    if (recovered.conversationUrl !== request.conversationUrl ||
        !hasRequiredTerminalMarkers(rawResponse, request.requiredTerminalMarkers)) {
...
async function executeCustomGemSubmit(cmd) {
  ...
  try {
    let tab;
    const tabReadyDeadline = Math.min(deadlineAt, Date.now() + 5_000);
  ...
  try {
    ...
    try {
      ...
      if (!submitAttempted) {
        ...
        return customGemFollowupFailure(
          request.requestId,
          code === 'deadline_exceeded' ? 'timeout' : 'prepare',
          code,
          customGemErrorMessage(code),
          {
            ...context,
