...
The current Studio path splits ownership deliberately: Mahiro Browser Control's Trusted build creates the exact inactive Gem tab and uploads the two hash-bound runner files through its reviewed debugger/file-chooser contract; Local Gemini Proxy receives only the resulting tab/upload receipt plus metadata, then verifies the visible attachments, types the bounded message, sends once, and returns one attributable response.

Publish exactly one metadata-only request to `claude/browser/command` with:

- `action: "gem_submit_v1"` and `version: "custom-gem-browser-submit-command-v1"`;
- the same canonical UUID in `id` and `requestId`, exact `gemUrl`/`currentUrl`, exact trusted-upload `tabId`, bounded `message`, `messageSha256`, `messageUtf8Bytes`, and `timeoutMs`;
- exactly two ordered metadata-only `sources`: `product_gallery`, then `creator_image`, each with basename `filename`, `mimeType`, `bytes`, and `sha256`—never base64, raw bytes, or local paths;
- one `custom-gem-source-upload-result-v1` receipt identity (`commandId` + `receiptId`).

The extension requires the exact pre-uploaded tab and URL, exactly two visible `gem-media-attachment` tiles bound to Browser Control's trusted sequential Product-then-Creator receipt (or legacy ordered filename markers), and one sender-ID-authenticated one-time attestation binding the request UUID, message hash, tab/URL, receipt, and ordered source metadata. It fingerprints and durably reserves the request in `chrome.storage.local` before consuming the attestation or attempting Send; altered in-flight duplicates fail conflict, a restart with an incomplete reservation returns ambiguous, and old completed results compact to non-expiring no-resend tombstones. The bounded 10,000-request store fails closed instead of evicting tombstones. It then requires an empty composer, one Send click, and one new complete stable assistant response. It never uploads files in this command, never falls back to an active/first tab, and treats post-Send timeout/disconnect as ambiguous.
...
- canonical UUID `requestId`, exact `gemUrl: "https://gemini.google.com/gem/d6f1958dff66"`, optional exact `tabId`, `message`, and `timeoutMs` in the inclusive 30s–360s range;
- exactly two ordered `images`: `product_gallery`, then `creator_image`. Each image must provide `filename`, `mimeType` (`image/jpeg|image/png|image/webp`), decoded raw-byte `bytes`, `sha256`, and canonical `base64`.
...
The extension enforces 3 MiB per image, 4 MiB total, 8192 UTF-8 message bytes, and a 64 KiB raw-response ceiling aligned with Studio import. It verifies decoded bytes, MIME signatures, and SHA-256 before tab work; uses only the explicit tab or creates one inactive exact-URL tab; never falls back to an active/first tab; and never logs the full command or base64. Per-tab/request locks and a request replay cache prevent duplicate execution. After Send, timeout/disconnect is `ambiguous`, the tab is quarantined, and the command is never resent automatically.

The result is `version: "custom-gem-browser-result-v1"` with the exact `requestId`, `state: "succeeded" | "failed" | "ambiguous"`, `gemId`, exact Gem `url`, `tabId`, `conversationUrl`, ordered source hashes/bytes, `messageSha256`, stage/error details, and success-only `responseSha256`, bounded `rawResponse`, and response UTF-8 byte count. The browser path is fail-closed: it requires stable composer/upload/send selectors, exactly two visible attachment tiles bound to Browser Control's trusted sequential Product-then-Creator receipt (or legacy ordered filename markers), a response/message baseline, one Send click, and one new complete stable assistant response. It returns no DOM, HTML, or unrelated page text.
