...
let learningFile = join(agentStateDir, `memory/learnings/${date}_${slug}.md`);
let count = 1;
while (existsSync(learningFile)) {
  learningFile = join(agentStateDir, `memory/learnings/${date}_${slug}-${count}.md`);
  count++;
}
...
appendFileSync(slugsFile, `${slug}: ${learningFile}\n`);
