...
## Portable CCC Security Policy

The current project's `.cocoindex_code/settings.yml` is the portable path
enforcement boundary. The bundled V2 sync helper owns only its marked block,
preserves unrelated settings, keeps security and noise patterns separate, and
supports read-only `--check`. It does not require a command wrapper, an upstream
patch, a global matcher, or a hidden project-external exclude file.

The credential baseline targets real dotenv files, provider/local settings,
credential directories, service-account files, and key material. JSON,
YAML/YML, TOML, XML, TXT, and durable `.agent-state` records remain eligible by
default. Explicit dotenv templates (`.env.example`, `.env.sample`, and
`.env.template`) are filename-allowed, explicitly included for indexing, and
must pass strict content scanning unless an unrelated explicit project exclude
blocks them.

For strict scanning, use Gitleaks `v8.30.1` at the managed path documented in
the parent skill. `scripts/ensure-gitleaks.py check` verifies the regular
non-symlink executable by its pinned hash and does not run it. A symlinked
ancestor fails closed before that classification, including when the final file
is valid. `ensure` may install that binary only when the path is missing.
Publish links the verified file only while the destination is still absent and
does not replace a destination that appears. Missing only; invalid blocks. A
symlinked ancestor or other invalid existing state fails closed with no network
and no replacement. Pass the platform binary SHA-256 from the
parent skill into the strict helper. A clean receipt binds the
scanner binary/version, reviewed rules/template, project root, settings,
policy, allowlist, and the current post-settings CCC candidate scope/content.
Policy-excluded security/noise paths are not opened merely to prove they are
excluded. Filename-only preflight is explicitly non-equivalent and never
substitutes for a strict receipt.

After any policy/settings change, run sync, filename-only preflight, strict
scan, reset the stale index, and rebuild. The Letta hook independently checks
the project settings and receipt before CCC file-access/indexing operations; it
does not mutate settings or patch CocoIndex.
