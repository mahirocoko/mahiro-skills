...
After initialization, resolve this installed `ccc` skill directory and run `sync-project-excludes.py`, then `preflight.py --check-settings`. Run `ensure-gitleaks.py check`. Run `ensure` only when the managed path is missing, then `check` again. Missing only; invalid blocks. A symlink, symlinked ancestor, non-executable file, wrong hash, wrong version, permission failure, or malformed path is not repaired. Publish does not replace a destination that appears before the verified file is linked into place. Create a clean strict receipt with `strict-gitleaks-scan.py scan`, then confirm it with `check`, using the managed path and the platform SHA-256 from the parent skill. Verify with `ccc doctor`, then run `ccc index`. Never chain `ccc init && ccc index`, and do not downgrade to filename-only when the strict gate fails. If the model test printed `[FAIL]` during `init`, edit `global_settings.yml` (and optionally add API keys under the commented `envs:` block) and verify with `ccc doctor` before indexing.
