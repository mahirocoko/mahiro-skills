...
When CocoIndex/`ccc` is available or already initialized in the target repo, generated `AGENTS.md` should include search guidance that prefers semantic search for broad/fuzzy codebase exploration and `rg` for exact search. It must also require filename-only preflight without opening suspected secret contents, forbid unchecked `ccc init && ccc index`, materialize the portable security/noise policy into the target project's `.cocoindex_code/settings.yml`, reject local embeddings as authorization to read secrets, and require stale indexes to be reset or safely rebuilt after exclusion-policy changes. If no such tooling exists, do not invent it as current reality.
