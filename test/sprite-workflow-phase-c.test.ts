...
describe("sprite workflow Phase C", () => {
  ...
  test("assembles deterministic approved atlases and validates them", () => {
    ...
    expect(run("assemble-approved-atlas.py", [manifest, "--output", a, "--atlas-manifest", am, "--layout", "row"]).status).toBe(0);
    expect(run("assemble-approved-atlas.py", [manifest, "--output", b, "--atlas-manifest", bm, "--layout", "row"]).status).toBe(0);
  ...
  test("refuses unapproved, hash-mismatched, traversal, and symlink atlas inputs", () => {
    ...
    const invoke = (value: object) => { const m = join(work, "m.json"); writeFileSync(m, JSON.stringify(value)); return run("assemble-approved-atlas.py", [m, "--output", join(work,"a.png"), "--atlas-manifest", join(work,"a.json")]); };
