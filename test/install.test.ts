...
describe("install", () => {
  ...
  test("surfaces bundle description when installing the default bundle", () => {
    ...
    try {
      ...
      expect(result.installed).toEqual(["asset-designer", "cocoindex-rules-init", "deep-research", "direct-cli", "forward", "gemini", "learn", "mac-calendar-booking", "mahiro-docs-rules-init", "mahiro-guidance-refine", "mahiro-style", "philosophy", "project", "recap", "rrr", "sprite-workflow", "uncodixify", "web-asset-prompts", "watch"]);
  ...
  test("copies direct-cli skill, preserved playbook, and paired command", () => {
    ...
    try {
      ...
      expect(readFileSync(sourceSkillPath, "utf8")).toContain("description: Direct executor playbook for using gemini CLI, Cursor CLI, Antigravity CLI, and Codex CLI through fresh tmux sessions");
      expect(readFileSync(sourceSkillPath, "utf8")).not.toContain("description: Mahiro Skill |");
      expect(readFileSync(sourceCommandPath, "utf8")).toContain("description: Direct executor playbook for using gemini CLI, Cursor CLI, Antigravity CLI, and Codex CLI through fresh tmux sessions");
      expect(readFileSync(sourceCommandPath, "utf8")).not.toContain("description: Mahiro Skill |");
      expect(readFileSync(installedSkillPath, "utf8")).toContain("description: Mahiro Skill | Direct executor playbook for using gemini CLI, Cursor CLI, Antigravity CLI, and Codex CLI through fresh tmux sessions");
      expect(readFileSync(installedCommandPath, "utf8")).toContain("description: Mahiro Skill | Direct executor playbook for using gemini CLI, Cursor CLI, Antigravity CLI, and Codex CLI through fresh tmux sessions");
  ...
  test("installs direct-cli under the gemini root with namespaced command", () => {
    ...
    try {
      ...
      expect(readFileSync(installedCommandPath, "utf8")).toContain(
        'description = "Mahiro Skill | Direct executor playbook for using gemini CLI, Cursor CLI, Antigravity CLI, and Codex CLI through fresh tmux sessions without going through the usual orchestration runtime. Use when you want a pane-first direct CLI lane, narrow current-worktree follow-up, or fresh-session recovery."',
      );
