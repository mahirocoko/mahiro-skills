...
describe("README", () => {
  ...
  test("documents install and usage truthfully without public-page noise", () => {
    ...
    expect(content).toContain("curl -fsSL https://raw.githubusercontent.com/mahirocoko/mahiro-skills/main/install.sh | bash -s -- --version v0.1.110 -- --agent opencode --scope global");
    ...
    expect(content).toContain("Human-readable skill writing: [`docs/authoring/human-readable-skill-writing.md`](./docs/authoring/human-readable-skill-writing.md)");
  ...
  test("documents the skill index and examples surfaces", () => {
    ...
    expect(index).toContain("`rrr` — Session retrospective with an optional gated non-canonical reference learning.");
