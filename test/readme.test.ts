...
describe("README", () => {
  ...
  test("documents install and usage truthfully without public-page noise", () => {
    ...
    expect(content).toContain("curl -fsSL https://raw.githubusercontent.com/mahirocoko/mahiro-skills/main/install.sh | bash -s -- --version v0.1.84 -- --agent opencode --scope global");
  ...
  test("documents the skill index and examples surfaces", () => {
    ...
    expect(index).toContain("`cocoindex-rules-init` — Repo-local CocoIndex Code rule bootstrapper with secret-safe index preflight.");
