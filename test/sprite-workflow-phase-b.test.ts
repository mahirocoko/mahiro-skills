...
describe("sprite workflow Phase B", () => {
  ...
  test("keeps legacy fixed horizontal and supports fixed row-major grids", () => {
    ...
    expect(result.status).toBe(0); const manifest = JSON.parse(readFileSync(join(out, "manifest.json"), "utf8")); expect(manifest.source.grid.order).toBe("row-major"); expect(manifest.frames.map((f: any) => f.index)).toEqual([0,1,2,3]);
