...
**If you have a Skill tool available**: Use it directly with `skill: "control-room-goals"` instead of reading the file manually.

**Otherwise**: Resolve the installed `control-room-goals/SKILL.md` under the current agent's configured skills root, then follow ALL instructions in it. Do not assume a source-checkout-relative `skills/...` path.
