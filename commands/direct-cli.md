...
Treat `--detach` as a skill-level Herdr-only mode after agents are visibly ready. Require or derive a unique `--job-id`. From an exact Letta parent pane, start `herdr-jobs.py` with explicit `--mode callback`; callback mode automatically launches its detached lifecycle guard and must not also receive a controller Monitor. The guard wakes the parent when a CLI reaches `idle`/`done` without an accepted final callback, while the silence deadline covers work that never settles. Use `--mode auto` only when watcher fallback is acceptable. If start reports watcher mode, or a Herdr lane was already launched without callback delivery, arm the current runtime's bounded background `Monitor` with `herdr-jobs.py wait <job-id> --json` or a receipt-bound lifecycle wait; never run that task-completion wait in the foreground. Reserve foreground waits for brief readiness/synchronous gates. Reject tmux detach and never claim the watcher itself injects a Letta conversation turn; do not emulate return by launching a second `letta -p` turn or typing into the main pane.
