...
Treat `--detach` as a skill-level Herdr-only mode after agents are visibly ready. Require or derive a unique `--job-id`. From an exact Letta parent pane, start `herdr-jobs.py` with explicit `--mode callback`; callback mode automatically launches its detached lifecycle guard and must not also receive a controller Monitor. The guard persists observed `working` evidence and requires a continuously stable `idle`/`done` window before warning about a missing final callback; a sequence change or transient terminal label is only advisory. The silence deadline covers work that never settles. A guard/deadline wake is not authorization for blind fallback capture: inspect the job and pane, then let callback `recover` synchronously capture only from the exact parent after dispatch completed, working was observed, and receipt-bound terminal evidence stayed stable, with a final state/sequence/callback recheck that defers when work or callback evidence remains active. Use `--mode auto` only when watcher fallback is acceptable. If start reports watcher mode, or a Herdr lane was already launched without callback delivery, arm the current runtime's bounded background `Monitor` with `herdr-jobs.py wait <job-id> --json` or a receipt-bound lifecycle wait; never run that task-completion wait in the foreground. Current Letta Code keeps that exact Monitor alive across Esc, SIGINT, and `abort_message`: a turn interruption is not cancellation, so retain its task ID, do not re-arm it after an interrupted turn, and close it explicitly with `TaskStop`. Reserve foreground waits for brief readiness/synchronous gates. Reject tmux detach and never claim the watcher itself injects a Letta conversation turn; do not emulate return by launching a second `letta -p` turn or typing into the main pane.
