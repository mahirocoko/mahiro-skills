...
Treat `--detach` as a skill-level Herdr-only mode after agents are visibly ready. Require or derive a unique `--job-id` and start the packaged durable watcher. If the current main runtime exposes a background task/monitor that reports completion into this conversation, arm it with `herdr-jobs.py wait <job-id> --json`; on completion, `collect`, synthesize, and answer. Otherwise use durable `list`/`show`/`collect` on later turns. Reject tmux detach and never claim the shell watcher itself injects a Letta conversation turn; do not emulate return by launching a second `letta -p` turn or typing into the main pane.
