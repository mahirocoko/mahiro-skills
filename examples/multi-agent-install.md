...
## Preview Cursor, Gemini CLI, Agy, Letta Code, and Pi targets

```bash
bun ./src/cli.ts plan project --agent cursor --agent gemini --agent agy --agent letta-code --agent pi --scope local
```

Equivalent comma-separated form:

```bash
bun ./src/cli.ts plan project --agent cursor,gemini,agy,letta-code,pi --scope local
```
